...
int write_dengo_data_to_file( dengo_field_data *field_data ){
    ...
    hid_t file_id = H5Fcreate("write_dengo_solution.h5", H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
   
    if (file_id < 0){
        fprintf(stderr, "Failed to open %s so dying.\n", "write_dengo_solution.h5");
        return(1);
    }
    ...
    double H2_1[dims];
    for (j = 0; j < dims; j++) {
        H2_1[j] = field_data->H2I_density[j]; 
    }
    ...
    double H2_2[dims];
    for (j = 0; j < dims; j++) {
        H2_2[j] = field_data->H2II_density[j];  
    }
    ...
    double H_2[dims];
    for (j = 0; j < dims; j++) {
        H_2[j] = field_data->HII_density[j]; 
    }
    ...
    double H_m0[dims];
    for (j = 0; j < dims; j++) {
        H_m0[j] = field_data->HM_density[j]; 
    }
    ...
    double He_1[dims];
    for (j = 0; j < dims; j++) {
        He_1[j] = field_data->HeI_density[j]; 
    }
    ...
    double He_2[dims];
    for (j = 0; j < dims; j++) {
        He_2[j] = field_data->HeII_density[j]; 
    }
    ...
    double He_3[dims];
    for (j = 0; j < dims; j++) {
        He_3[j] = field_data->HeIII_density[j]; 
    }
    ...
    // need a module that calculate hydro-quantities
    // like gamma, temperature, pressure
...
int read_init_data_to_dengo( dengo_field_data *field_data ){
    ...
    // this reads the rates h5 data generated by dengo
    // initialize a dengo_field_data object;
    // we will fill it up with the data read from 
    // h5 initial conditions, or supplied one.
...
//    }
    ...
    field_data->H2I_density = (double *) malloc(dims * sizeof(double));
    field_data->H2II_density = (double *) malloc(dims * sizeof(double));
    field_data->HI_density = (double *) malloc(dims * sizeof(double));
    field_data->HII_density = (double *) malloc(dims * sizeof(double));
    field_data->HM_density = (double *) malloc(dims * sizeof(double));
    field_data->HeI_density = (double *) malloc(dims * sizeof(double));
    field_data->HeII_density = (double *) malloc(dims * sizeof(double));
    field_data->HeIII_density = (double *) malloc(dims * sizeof(double));
    field_data->de_density = (double *) malloc(dims * sizeof(double));
    field_data->ge_density = (double *) malloc(dims * sizeof(double));
    ...
    for (j = 0; j < dims; j++) {
        field_data->H2I_density[j] = tics[j]; 
        if(j==0) {
    ...
    for (j = 0; j < dims; j++) {
        field_data->H2II_density[j] = tics[j]; 
        if(j==0) {
    ...
    for (j = 0; j < dims; j++) {
        field_data->HI_density[j] = tics[j]; 
        if(j==0) {
    ...
    for (j = 0; j < dims; j++) {
        field_data->HII_density[j] = tics[j]; 
        if(j==0) {
    ...
    for (j = 0; j < dims; j++) {
        field_data->HM_density[j] = tics[j]; 
        if(j==0) {
    ...
    for (j = 0; j < dims; j++) {
        field_data->HeI_density[j] = tics[j]; 
        if(j==0) {
    ...
    for (j = 0; j < dims; j++) {
        field_data->HeII_density[j] = tics[j]; 
        if(j==0) {
    ...
    for (j = 0; j < dims; j++) {
        field_data->HeIII_density[j] = tics[j]; 
        if(j==0) {
...
int cvdls_9species_solve_chemistry_dt( dengo_field_data *field_data, 
        cvdls_9species_data *data, double dt )
{   
    int i, j;
    int N = 10; // filled in by dengo number of species 
    int dims = field_data->ncells  ; // total number of strips to be evalulated
    ...
    for ( int d = 0; d< dims; d++  ){
        ...
        input[j] = field_data->H2I_density[d];
        input[j] /= 2.01588;
        atol[j]   = input[j]*1e-9;
        
        j++;
        
        input[j] = field_data->H2II_density[d];
        input[j] /= 2.01588;
        atol[j]   = input[j]*1e-9;

        j++;
    
        input[j] = field_data->HI_density[d];
        input[j] /= 1.00794;
        atol[j]   = input[j]*1e-9;

        j++;

        input[j] = field_data->HII_density[d];
        input[j] /= 2.01588;
        atol[j]   = input[j]*1e-9;

        j++;
        
        input[j] = field_data->HM_density[d];
        input[j] /= 2.01588;
        atol[j]   = input[j]*1e-9;

        j++;
    
        input[j] = field_data->HeI_density[d];
        input[j] /= 2.01588;
        atol[j]   = input[j]*1e-9;

        j++;
        
        input[j] = field_data->HeII_density[d];
        input[j] /= 2.01588;
        atol[j]   = input[j]*1e-9;

        j++;
        
        input[j] = field_data->HeIII_density[d];
        input[j] /= 2.01588;
        atol[j]   = input[j]*1e-9;
    ...
    ttot = dengo_evolve_cvdls_9species(dt, dt, z, input, rtol, atol, dims, data);
    
    // pipe input back to the pointers data_structures;
    ...
    for ( int d = 0; d< dims; d++  ){
        ...
        field_data->H2I_density[d] = input[j] * 2.01588;
        j++;
        
        field_data->H2II_density[d] = input[j] * 2.01588;
        j++;
    
        field_data->HI_density[d] = input[j] *2.01588;
        j++;
        
        field_data->HII_density[d] = input[j] *2.01588;
        j++;
        
        field_data->HM_density[d]  = input[j] *2.01588;
        j++;

    
        field_data->HeI_density[d] = input[j] *2.01588;
        j++;
        
        field_data->HeII_density[d] = input[j]*2.01588;
        j++;
        
        field_data->HeIII_density[d] = input[j]*2.01588;
        j++;
        
        field_data->de_density[d] = input[j] * 2.01588;
        j++;
        
        field_data->ge_density[d] = input[j] * 2.01588;
        j++;
...
double dengo_evolve_cvdls_9species (double dtf, double &dt, double z, double *input,
    ...
    void *setup_cvode_solver( rhs_f f, jac_f Jac, double abstol, int NEQ, cvdls_9species_data *data , SUNLinearSolver LS, SUNMatrix A);
    int cvode_solver( void *cvode_mem, double *output, int NEQ, double *dt);
    ...
    cvode_mem = setup_cvode_solver( f, jf, abstol, N, data, LS, A);
    //if (flag > 0) {
    //    fprintf(stderr, "falied to setup the CVODE solver \n");
    //    return 1;
    //}
    ...
    for (int d = 0; d < dims; d++){
        ...
        while (ttot < dtf) { 
            ...
            flag = cvode_solver( cvode_mem, y, N, &dt);
            ...
            if ( flag == 0) {
                ...
                // rescale the scale, inv_scale in the data 
                for (i = 0; i < N; i++){
                        ...
                        double *input, int nstrip, int nchem)
{
    ...
    for (i = 0; i<nstrip; i++) {
        ...
        if (Tnew < data->bounds[0]) {
            data->Ts[i] = data->bounds[0];
        } else if (Tnew > data->bounds[1]) {
            data->Ts[i] = data->bounds[1];
        }
        data->logTs[i] = log(Tnew);
        data->invTs[i] = 1.0 / Tnew;
	    data->dTs_ge[i] = 
                    ...
                    int nstrip)
{
    ...
    double zbound0 = data->z_bounds[0];
    double zbound1 = data->z_bounds[1];
    double current_z = data->current_z;

    /*
    if (( current_z > zbound0 ) && ( current_z < zbound1 )) {
...
void cvdls_9species_interpolate_gamma(cvdls_9species_data *sdata,
                    int i)
{   
    ...
    cvdls_9species_data *data = sdata;
