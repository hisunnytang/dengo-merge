...
int write_dengo_data_to_file( dengo_field_data *field_data ){
    ...
    file_id = H5Fcreate("write_dengo_solution.h5", H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
    hsize_t dimsarr[1];
    dimsarr[0] = dims;
    i = 0;
...
int read_init_data_to_dengo(int argc, char** argv){
    ...
    field_data = new dengo_field_data; 
    ...
    for (j = 0; j < dims; j++) {
        ...
        if(j==0) {
            fprintf(stderr, "H2_1[0] = %0.3g", tics[j] );
        }
    }
    i++;
    ...
    for (j = 0; j < dims; j++) {
        field_data->HeII_density = tics[j]; 
        if(j==0) {
    ...
    for (j = 0; j < dims; j++) {
        field_data->HeIII_density = tics[j]; 
        if(j==0) {
...
int cvdls_9speies_solve_chemistry_dt( dengo_field_data *field_data, cvdls_9species_data *data, double dt )
{   
    int i, j;
    int N = 10; // filled in by dengo number of species 
    int dims = 100  ; // total number of strips to be evalulated
