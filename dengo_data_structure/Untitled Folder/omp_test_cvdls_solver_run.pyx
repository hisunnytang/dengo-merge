...
cdef extern from "omp_test_cvdls_solver.h":
    cdef int _MAX_NCELLS  "MAX_NCELLS"
    cdef int _NSPECIES "NSPECIES"

    ctypedef struct omp_test_cvdls_data:
        double dbin
        double idbin
        double bounds[2]
        int nbins
    ...
    omp_test_cvdls_data *omp_test_cvdls_setup_data(int *NumberOfFields,
            char ***FieldNames)
    void omp_test_cvdls_read_rate_tables(omp_test_cvdls_data*)
    void omp_test_cvdls_read_cooling_tables(omp_test_cvdls_data*)
    void omp_test_cvdls_read_gamma(omp_test_cvdls_data*)

    double dengo_evolve_omp_test_cvdls (double dtf, double &dt, double z,
                                         double *input, double *rtol,
                                         double *atol, int dims,
                                         omp_test_cvdls_data *data)

    # Declare the Jacobian and RHS function
    
    int calculate_jacobian_omp_test_cvdls(long int N, realtype t,
                   N_Vector y, N_Vector fy, DlsMat J, void *user_data,
                   N_Vector tmp1, N_Vector tmp2, N_Vector tmp3);

    int calculate_rhs_omp_test_cvdls(realtype t, N_Vector y, 
...
def main_run_omp_test_cvdls():
    t1 = time.time()
    omp_test_cvdls_main(0, NULL)
    t2 = time.time()
    print "Total elapsed time: %0.3e" % (t2-t1)

def run_omp_test_cvdls(ics, double tf, int niter = 10000,
    ...
    cdef omp_test_cvdls_data *data = omp_test_cvdls_setup_data(NULL, NULL)
    cdef rhs_f f = calculate_rhs_omp_test_cvdls
    cdef jac_f jf = calculate_jacobian_omp_test_cvdls

    cdef double ttot = 0.0
    cdef int status
    # Allocate some temporary data
    # Now we manually evolve
    #ttot = dengo_evolve_omp_test_cvdls(tf, dt, input, rtol, atol, dims, data)
    data.current_z = z
