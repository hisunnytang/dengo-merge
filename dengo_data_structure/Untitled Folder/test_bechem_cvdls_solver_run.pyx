...
cdef extern from "test_bechem_cvdls_solver.h":
    cdef int _MAX_NCELLS  "MAX_NCELLS"
    cdef int _NSPECIES "NSPECIES"
    ctypedef struct test_bechem_cvdls_data:
        double dbin
        double idbin
        double bounds[2]
        int nbins
    ...
    test_bechem_cvdls_data *test_bechem_cvdls_setup_data(int *NumberOfFields,
            char ***FieldNames)
    void test_bechem_cvdls_read_rate_tables(test_bechem_cvdls_data*)
    void test_bechem_cvdls_read_cooling_tables(test_bechem_cvdls_data*)
    double dengo_evolve_test_bechem_cvdls (double dtf, double &dt, double z,
                                         double *input, double *rtol,
                                         double *atol, int dims,
                                         test_bechem_cvdls_data *data)
...
def main_run_test_bechem_cvdls():
    t1 = time.time()
    test_bechem_cvdls_main(0, NULL)
    t2 = time.time()
    print "Total elapsed time: %0.3e" % (t2-t1)

def run_test_bechem_cvdls(ics, double tf, int niter = 10000,
    ...
    cdef test_bechem_cvdls_data *data = test_bechem_cvdls_setup_data(NULL, NULL)
    cdef rhs_f f = calculate_rhs_test_bechem_cvdls
    cdef jac_f jf = calculate_jacobian_test_bechem_cvdls
    ...
    #ttot = dengo_evolve_test_bechem_cvdls(tf, dt, input, rtol, atol, dims, data)
    data.current_z = z
    ...
    if intermediate:
        rv_t["H2_1"] = H2_1_int[:niter] * 2.01588
        rv_t["H2_2"] = H2_2_int[:niter] * 2.01588
        rv_t["H_1"] = H_1_int[:niter] * 1.00794
        rv_t["H_2"] = H_2_int[:niter] * 1.00794
        rv_t["H_m0"] = H_m0_int[:niter] * 1.00794
        rv_t["He_1"] = He_1_int[:niter] * 4.002602
        rv_t["He_2"] = He_2_int[:niter] * 4.002602
        rv_t["He_3"] = He_3_int[:niter] * 4.002602
