...
#include "test_bechem_cvdls_solver.h"

test_bechem_cvdls_data *test_bechem_cvdls_setup_data(
    int *NumberOfFields, char ***FieldNames)
{
    int i;

    test_bechem_cvdls_data *data = (test_bechem_cvdls_data *) malloc(sizeof(test_bechem_cvdls_data));
    ...
    test_bechem_cvdls_read_rate_tables(data);
    fprintf(stderr, "Successfully read in rate tables.\n");

    test_bechem_cvdls_read_cooling_tables(data);
    fprintf(stderr, "Successfully read in cooling rate tables.\n");

    test_bechem_cvdls_read_gamma(data);
...
int test_bechem_cvdls_main(int argc, char** argv)
{
    test_bechem_cvdls_data *data = test_bechem_cvdls_setup_data(NULL, NULL);

    /* Initial conditions */

    hid_t file_id = H5Fopen("test_bechem_cvdls_initial_conditions.h5", H5F_ACC_RDONLY, H5P_DEFAULT);
    if (file_id < 0) {fprintf(stderr, "Failed to open "
        "test_bechem_cvdls_initial_conditions.h5 so dying.\n");
        return(1);}
    ...
    double ttot;
    ttot = dengo_evolve_test_bechem_cvdls(dtf, dt, z, input, rtol, atol, dims, data);

    /* Write results to HDF5 file */
    file_id = H5Fcreate("test_bechem_cvdls_solution.h5", H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
    hsize_t dimsarr[1];
    dimsarr[0] = dims;
    i = 0;
...
double dengo_evolve_test_bechem_cvdls (double dtf, double &dt, double z, double *input,
            double *rtol, double *atol, long long dims, test_bechem_cvdls_data *data) {
    int i, j;
    hid_t file_id;
    /* fprintf(stderr, "  ncells = % 3i\n", (int) dims); */
    ...
    rhs_f f = calculate_rhs_test_bechem_cvdls;
    jac_f jf = calculate_jacobian_test_bechem_cvdls;
    if (dt < 0) dt = dtf / 1e5;
    data->current_z = z;
    int niter = 0;
    int siter = 0;
...
void test_bechem_cvdls_read_rate_tables(test_bechem_cvdls_data *data)
{
    hid_t file_id = H5Fopen("test_bechem_cvdls_tables.h5", H5F_ACC_RDONLY, H5P_DEFAULT);
...
void test_bechem_cvdls_read_cooling_tables(test_bechem_cvdls_data *data)
{
 ...
 void test_bechem_cvdls_read_gamma(test_bechem_cvdls_data *data)
{
...
void test_bechem_cvdls_calculate_temperature(test_bechem_cvdls_data *data,
                        double *input, int nstrip, int nchem)
{
    ...
    for (i = 0; i<nstrip; i++) {
        ...
        double gammaH2_1, gammaH2_2, dgammaH2_1_dT, dgammaH2_2_dT;
        double dge, dge_dT;
        while ( Tdiff > 0.1 ){
        
        T = data->Ts[i];
        
        // test_bechem_cvdls_interpolate_gamma(data, i);
...
void test_bechem_cvdls_interpolate_rates(test_bechem_cvdls_data *data,
                    int nstrip)
{
...
void test_bechem_cvdls_interpolate_gamma(test_bechem_cvdls_data *data,
                    int i)
{   
...
int calculate_rhs_test_bechem_cvdls(double *input, double *rhs, int nstrip,
                  int nchem, void *sdata)
{
    /* We iterate over all of the rates */
    /* Calculate temperature first */
    test_bechem_cvdls_data *data = (test_bechem_cvdls_data*)sdata;
    int i, j;
    test_bechem_cvdls_calculate_temperature(data, input, nstrip, nchem);

    test_bechem_cvdls_interpolate_rates(data, nstrip);
    ...
    for (i = 0; i<nstrip; i++) {
        ...
        // Species: H2_1
        // 
        rhs[j] = k08[i]*H_1*H_m0 + k10[i]*H2_2*H_1 - k11[i]*H2_1*H_2 - k12[i]*H2_1*de - k13[i]*H2_1*H_1 + k19[i]*H2_2*H_m0 + k21[i]*H2_1*pow(H_1, 2) + k22[i]*pow(H_1, 3);
        ...
        // Species: H_1
        // 
        rhs[j] = -k01[i]*H_1*de + k02[i]*H_2*de - k07[i]*H_1*de - k08[i]*H_1*H_m0 - k09[i]*H_1*H_2 - k10[i]*H2_2*H_1 + k11[i]*H2_1*H_2 + 2*k12[i]*H2_1*de + 2*k13[i]*H2_1*H_1 + k14[i]*H_m0*de + k15[i]*H_1*H_m0 + 2*k16[i]*H_2*H_m0 + 2*k18[i]*H2_2*de + k19[i]*H2_2*H_m0 - 2*k21[i]*H2_1*pow(H_1, 2) - 2*k22[i]*pow(H_1, 3);
...
int calculate_jacobian_test_bechem_cvdls(double *input, double *Joutput,
        int nstrip, int nchem, void *sdata)
{
	...
	ge = input[j];
        ...
        //
            ...
            j++;
            // ge by H2_1
            Joutput[j] = -H2_1*gloverabel08_gaH2[i]*pow(gloverabel08_h2lte[i], 2)/(pow(gloverabel08_h2lte[i]/(H2_1*gloverabel08_gaH2[i] + H_1*gloverabel08_gaHI[i] + H_2*gloverabel08_gaHp[i] + He_1*gloverabel08_gaHe[i] + de*gloverabel08_gael[i]) + 1.0, 2)*pow(H2_1*gloverabel08_gaH2[i] + H_1*gloverabel08_gaHI[i] + H_2*gloverabel08_gaHp[i] + He_1*gloverabel08_gaHe[i] + de*gloverabel08_gael[i], 2)) - 0.5*H_1*h2formation_h2mcool[i]*1.0/(h2formation_ncrn[i]/(H2_1*h2formation_ncrd2[i] + H_1*h2formation_ncrd1[i]) + 1.0) - gloverabel08_h2lte[i]/(gloverabel08_h2lte[i]/(H2_1*gloverabel08_gaH2[i] + H_1*gloverabel08_gaHI[i] + H_2*gloverabel08_gaHp[i] + He_1*gloverabel08_gaHe[i] + de*gloverabel08_gael[i]) + 1.0) + 0.5*h2formation_ncrd2[i]*h2formation_ncrn[i]*pow(h2formation_ncrn[i]/(H2_1*h2formation_ncrd2[i] + H_1*h2formation_ncrd1[i]) + 1.0, -2.0)*(-H2_1*H_1*h2formation_h2mcool[i] + pow(H_1, 3)*h2formation_h2mheat[i])/pow(H2_1*h2formation_ncrd2[i] + H_1*h2formation_ncrd1[i], 2);
        ...
        //
            ...
            j++;
            // ge by H_1
            Joutput[j] = -H2_1*gloverabel08_gaHI[i]*pow(gloverabel08_h2lte[i], 2)/(pow(gloverabel08_h2lte[i]/(H2_1*gloverabel08_gaH2[i] + H_1*gloverabel08_gaHI[i] + H_2*gloverabel08_gaHp[i] + He_1*gloverabel08_gaHe[i] + de*gloverabel08_gael[i]) + 1.0, 2)*pow(H2_1*gloverabel08_gaH2[i] + H_1*gloverabel08_gaHI[i] + H_2*gloverabel08_gaHp[i] + He_1*gloverabel08_gaHe[i] + de*gloverabel08_gael[i], 2)) - ceHI_ceHI[i]*de - ciHI_ciHI[i]*de + 0.5*h2formation_ncrd1[i]*h2formation_ncrn[i]*pow(h2formation_ncrn[i]/(H2_1*h2formation_ncrd2[i] + H_1*h2formation_ncrd1[i]) + 1.0, -2.0)*(-H2_1*H_1*h2formation_h2mcool[i] + pow(H_1, 3)*h2formation_h2mheat[i])/pow(H2_1*h2formation_ncrd2[i] + H_1*h2formation_ncrd1[i], 2) + 0.5*(-H2_1*h2formation_h2mcool[i] + 3*pow(H_1, 2)*h2formation_h2mheat[i])*1.0/(h2formation_ncrn[i]/(H2_1*h2formation_ncrd2[i] + H_1*h2formation_ncrd1[i]) + 1.0);
