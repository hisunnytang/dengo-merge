...
cdef extern from "test_cvdls_solver.h":
    cdef int _MAX_NCELLS  "MAX_NCELLS"
    cdef int _NSPECIES "NSPECIES"

    ctypedef struct test_cvdls_data:
        double dbin
        double idbin
        double bounds[2]
        int nbins
    ...
    test_cvdls_data *test_cvdls_setup_data(int *NumberOfFields,
            char ***FieldNames)
    void test_cvdls_read_rate_tables(test_cvdls_data*)
    void test_cvdls_read_cooling_tables(test_cvdls_data*)
    void test_cvdls_read_gamma(test_cvdls_data*)

    double dengo_evolve_test_cvdls (double dtf, double &dt, double z,
                                         double *input, double *rtol,
                                         double *atol, int dims,
                                         test_cvdls_data *data, double *temp)

    # Declare the Jacobian and RHS function
    
    int calculate_jacobian_test_cvdls(long int N, realtype t,
                   N_Vector y, N_Vector fy, DlsMat J, void *user_data,
                   N_Vector tmp1, N_Vector tmp2, N_Vector tmp3);

    int calculate_rhs_test_cvdls(realtype t, N_Vector y, 
                    N_Vector ydot, void *user_data);
    
    void test_cvdls_calculate_temperature(test_cvdls_data *data, double *input, int nstrip, int nchem);
   
       
    int ensure_electron_consistency(double *input, int nstrip, int nchem);
    

def main_run_test_cvdls():
    t1 = time.time()
    test_cvdls_main(0, NULL)
    t2 = time.time()
    print "Total elapsed time: %0.3e" % (t2-t1)

def run_test_cvdls(ics, double tf, int niter = 10000,
    ...
    cdef test_cvdls_data *data = test_cvdls_setup_data(NULL, NULL)
    cdef rhs_f f = calculate_rhs_test_cvdls
    cdef jac_f jf = calculate_jacobian_test_cvdls

    cdef double ttot = 0.0
    cdef int status
    # Allocate some temporary data
    # Now we manually evolve
    #ttot = dengo_evolve_test_cvdls(tf, dt, input, rtol, atol, dims, data)
    data.current_z = z
    ...
    for iter in range(niter):
        dt_local = dengo_evolve_test_cvdls( dt0[0], dt[0], z, input, rtol, atol, dims, data, temp  )
        if dt_local > 0:
            status = 0
