...
void *setup_cvode_solver( rhs_f f, jac_f Jac, double abstol , int , 
        omp_test_cvdls_data *data, SUNLinearSolver , SUNMatrix , N_Vector );
int cvode_solver( void *cvode_mem, double *output, int NEQ, double *dt, omp_test_cvdls_data * data, N_Vector y);
    ...
    int *NumberOfFields, char ***FieldNames)
{
    ...
    omp_test_cvdls_data *data = (omp_test_cvdls_data *) malloc(sizeof(omp_test_cvdls_data));
    ...
    omp_test_cvdls_read_rate_tables(data);
    fprintf(stderr, "Successfully read in rate tables.\n");

    omp_test_cvdls_read_cooling_tables(data);
    fprintf(stderr, "Successfully read in cooling rate tables.\n");
    
    omp_test_cvdls_read_gamma(data);
...
int omp_test_cvdls_main(int argc, char** argv)
{
    omp_test_cvdls_data *data = omp_test_cvdls_setup_data(NULL, NULL);

    /* Initial conditions */

    hid_t file_id = H5Fopen("omp_test_cvdls_initial_conditions.h5", H5F_ACC_RDONLY, H5P_DEFAULT);
    if (file_id < 0) {fprintf(stderr, "Failed to open "
        "omp_test_cvdls_initial_conditions.h5 so dying.\n");
        return(1);}
    ...
    double ttot;
    ttot = dengo_evolve_omp_test_cvdls(dtf, dt, z, input, rtol, atol, dims, data);

    /* Write results to HDF5 file */
    file_id = H5Fcreate("omp_test_cvdls_solution.h5", H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
    hsize_t dimsarr[1];
    dimsarr[0] = dims;
    i = 0;
...
double dengo_evolve_omp_test_cvdls (double dtf, double &dt, double z, double *input,
    ...
    rhs_f f = calculate_rhs_omp_test_cvdls;
    jac_f jf = calculate_jacobian_omp_test_cvdls;
    if (dt < 0) dt = dtf / 1e0;
    data->current_z = z;
    int niter = 0;
    int siter = 0;
...
void omp_test_cvdls_read_rate_tables(omp_test_cvdls_data *data)
{
    hid_t file_id = H5Fopen("omp_test_cvdls_tables.h5", H5F_ACC_RDONLY, H5P_DEFAULT);
...
void omp_test_cvdls_read_cooling_tables(omp_test_cvdls_data *data)
{
...
void omp_test_cvdls_read_gamma(omp_test_cvdls_data *data)
{
...
void omp_test_cvdls_calculate_temperature(omp_test_cvdls_data *data,
                        double *input, int nstrip, int nchem)
{
    ...
    while (Tdiff > 0.1 ){
        ...
        omp_test_cvdls_interpolate_gamma(data, i);
...
void omp_test_cvdls_interpolate_rates(omp_test_cvdls_data *data,
                    int nstrip)
{
...
void omp_test_cvdls_interpolate_gamma(omp_test_cvdls_data *data,
                    int i)
{   
...
int calculate_jacobian_omp_test_cvdls
                                        ( realtype t,
                                        ...
                                        N_Vector tmp3)
{
    ...
    omp_test_cvdls_data *data = (omp_test_cvdls_data*)user_data; 
    ...
    omp_test_cvdls_calculate_temperature(data, y_arr, nstrip, nchem);
    omp_test_cvdls_interpolate_rates(data, nstrip);
...
int calculate_rhs_omp_test_cvdls(realtype t, N_Vector y, N_Vector ydot, void *user_data)
{
    omp_test_cvdls_data *data = (omp_test_cvdls_data* ) user_data;
    int i, j;
...
int read_init_data_to_dengo( dengo_field_data *field_data ){
    ...
    char const *filename;
    filename = "omp_test_cvdls_initial_conditions.h5";
...
int write_dengo_data_to_file( dengo_field_data *field_data ){
    ...
    hid_t file_id = H5Fcreate("omp_test_cvdls_solution.h5", H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
    if (file_id < 0){
        fprintf(stderr, "failed to open %s so dying. \n", "omp_test_cvdls_solution.h5");
    }
    ...
    double H2_1[dims];
    for ( j = 0; j < dims; j++ ){
        H2_1[j] = field_data->H2_1_density[j];
    ...
    double H2_2[dims];
    for ( j = 0; j < dims; j++ ){
        H2_2[j] = field_data->H2_2_density[j];
    ...
    double H_1[dims];
    for ( j = 0; j < dims; j++ ){
        H_1[j] = field_data->H_1_density[j];
    ...
    double H_2[dims];
    for ( j = 0; j < dims; j++ ){
        H_2[j] = field_data->H_2_density[j];
    ...
    double H_m0[dims];
    for ( j = 0; j < dims; j++ ){
        H_m0[j] = field_data->H_m0_density[j];
    ...
    double He_1[dims];
    for ( j = 0; j < dims; j++ ){
        He_1[j] = field_data->He_1_density[j];
    ...
    double He_2[dims];
    for ( j = 0; j < dims; j++ ){
        He_2[j] = field_data->He_2_density[j];
    ...
    double He_3[dims];
    for ( j = 0; j < dims; j++ ){
        He_3[j] = field_data->He_3_density[j];
...
double dengo_evolve_omp_test_cvdls_field_data (double dtf, double &dt, double z, dengo_field_data *field_data,
            double rtol, double abstol, long long dims, omp_test_cvdls_data *data) {
    int i, j;
    /* fprintf(stderr, "  ncells = % 3i\n", (int) dims); */
    ...
    #pragma omp parallel num_threads (4) private (A, LS, cvode_mem, threadID, y_vec)
    {
        y_vec = NULL;
        y_vec = N_VNew_Serial(N);
        for ( i = 0; i < N; i++){
    ...
    for (int d = 0; d < dims; d++){
        ...
        j = 0;
        // Species: H2_1
        data->scale    [threadID][j] = field_data->H2_1_density[d]/ 2.01588;
        data->inv_scale[threadID][j] = 1.0/ data->scale[threadID][j];
        j++;
        
        // Species: H2_2
        data->scale    [threadID][j] = field_data->H2_2_density[d]/ 2.01588;
        data->inv_scale[threadID][j] = 1.0/ data->scale[threadID][j];
        j++;
        
        // Species: H_1
        data->scale    [threadID][j] = field_data->H_1_density[d]/ 1.00794;
        data->inv_scale[threadID][j] = 1.0/ data->scale[threadID][j];
        j++;
        
        // Species: H_2
        data->scale    [threadID][j] = field_data->H_2_density[d]/ 1.00794;
        data->inv_scale[threadID][j] = 1.0/ data->scale[threadID][j];
        j++;
        
        // Species: H_m0
        data->scale    [threadID][j] = field_data->H_m0_density[d]/ 1.00794;
        data->inv_scale[threadID][j] = 1.0/ data->scale[threadID][j];
        j++;
        
        // Species: He_1
        data->scale    [threadID][j] = field_data->He_1_density[d]/ 4.002602;
        data->inv_scale[threadID][j] = 1.0/ data->scale[threadID][j];
        j++;
        
        // Species: He_2
        data->scale    [threadID][j] = field_data->He_2_density[d]/ 4.002602;
        data->inv_scale[threadID][j] = 1.0/ data->scale[threadID][j];
        j++;
        
        // Species: He_3
        data->scale    [threadID][j] = field_data->He_3_density[d]/ 4.002602;
        data->inv_scale[threadID][j] = 1.0/ data->scale[threadID][j];
        j++;
        
        // Species: de
        data->scale    [threadID][j] = field_data->de_density[d]/ 1.0;
        data->inv_scale[threadID][j] = 1.0/ data->scale[threadID][j];
        j++;
        
        // Species: ge
        data->scale    [threadID][j] = field_data->ge_density[d]/ 1.0;
        data->inv_scale[threadID][j] = 1.0/ data->scale[threadID][j];
        j++;
        ...
        // Species: H2_1
        field_data->H2_1_density[d] = 2.01588 * data->scale[threadID][j];
        j ++;
        
        // Species: H2_2
        field_data->H2_2_density[d] = 2.01588 * data->scale[threadID][j];
        j ++;
        
        // Species: H_1
        field_data->H_1_density[d] = 1.00794 * data->scale[threadID][j];
        j ++;
        
        // Species: H_2
        field_data->H_2_density[d] = 1.00794 * data->scale[threadID][j];
        j ++;
        
        // Species: H_m0
        field_data->H_m0_density[d] = 1.00794 * data->scale[threadID][j];
        j ++;
        
        // Species: He_1
        field_data->He_1_density[d] = 4.002602 * data->scale[threadID][j];
        j ++;
        
        // Species: He_2
        field_data->He_2_density[d] = 4.002602 * data->scale[threadID][j];
        j ++;
        
        // Species: He_3
        field_data->He_3_density[d] = 4.002602 * data->scale[threadID][j];
        j ++;
        
        // Species: de
        field_data->de_density[d] = 1.0 * data->scale[threadID][j];
        j ++;
        
        // Species: ge
        field_data->ge_density[d] = 1.0 * data->scale[threadID][j];
        j ++;
