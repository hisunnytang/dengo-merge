    ...
    int *NumberOfFields, char ***FieldNames)
{
    ...
    new_data *data = (new_data *) malloc(sizeof(new_data));
    ...
    new_read_rate_tables(data);
    fprintf(stderr, "Successfully read in rate tables.\n");

    new_read_cooling_tables(data);
...
{
    new_data *data = new_setup_data(NULL, NULL);

    /* Initial conditions */

    hid_t file_id = H5Fopen("new_initial_conditions.h5", H5F_ACC_RDONLY, H5P_DEFAULT);
    if (file_id < 0) {fprintf(stderr, "Failed to open "
        "new_initial_conditions.h5 so dying.\n");
        return(1);}
    ...
    double ttot;
    ttot = dengo_evolve_new(dtf, dt, z, input, rtol, atol, dims, data);

    /* Write results to HDF5 file */
    file_id = H5Fcreate("new_solution.h5", H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
    hsize_t dimsarr[1];
    dimsarr[0] = dims;
    i = 0;
...
double dengo_evolve_new (double dtf, double &dt, double z, double *input,
            double *rtol, double *atol, long long dims, new_data *data) {
    int i, j;
    hid_t file_id;
    /* fprintf(stderr, "  ncells = % 3i\n", (int) dims); */
    ...
    rhs_f f = calculate_rhs_new;
    jac_f jf = calculate_jacobian_new;
    if (dt < 0) dt = dtf / 1e5;
    data->current_z = z;
    int niter = 0;
    int siter = 0;
...
void new_read_rate_tables(new_data *data)
{
    hid_t file_id = H5Fopen("new_tables.h5", H5F_ACC_RDONLY, H5P_DEFAULT);
...
void new_read_cooling_tables(new_data *data)
{
...
void new_read_gamma(new_data *data)
{
...
void new_calculate_temperature(new_data *data,
...
void new_interpolate_rates(new_data *data,
                    int nstrip)
{
...
void new_interpolate_gamma(new_data *data,
                    int i)
{   
                                        ...
                                        N_Vector tmp3)
{
    ...
    new_data *data = (new_data*)user_data; 
    ...
    new_calculate_temperature(data, y_arr, nstrip, nchem);

    new_interpolate_rates(data, nstrip);
...
int calculate_rhs_new(realtype t, N_Vector y, N_Vector ydot, void *user_data)
{
    new_data *data = (new_data* ) user_data;
    int i, j;
