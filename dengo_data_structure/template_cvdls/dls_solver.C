...
dls_data *dls_setup_data(
    int *NumberOfFields, char ***FieldNames)
{
    int i;

    dls_data *data = (dls_data *) malloc(sizeof(dls_data));
    ...
    dls_read_rate_tables(data);
    fprintf(stderr, "Successfully read in rate tables.\n");

    dls_read_cooling_tables(data);
...
{
    dls_data *data = dls_setup_data(NULL, NULL);

    /* Initial conditions */

    hid_t file_id = H5Fopen("dls_initial_conditions.h5", H5F_ACC_RDONLY, H5P_DEFAULT);
    if (file_id < 0) {fprintf(stderr, "Failed to open "
        "dls_initial_conditions.h5 so dying.\n");
        return(1);}
    ...
    double ttot;
    ttot = dengo_evolve_dls(dtf, dt, z, input, rtol, atol, dims, data);

    /* Write results to HDF5 file */
    file_id = H5Fcreate("dls_solution.h5", H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
    hsize_t dimsarr[1];
    dimsarr[0] = dims;
    i = 0;
...
double dengo_evolve_dls (double dtf, double &dt, double z, double *input,
            double *rtol, double *atol, long long dims, dls_data *data) {
    int i, j;
    hid_t file_id;
    /* fprintf(stderr, "  ncells = % 3i\n", (int) dims); */
    ...
    rhs_f f = calculate_rhs_dls;
    jac_f jf = calculate_jacobian_dls;
    if (dt < 0) dt = dtf / 1e5;
    data->current_z = z;
    int niter = 0;
    int siter = 0;
    ...
    double *dt_now = (double *) alloca(sizeof(double)) ;
    dt_now[0] = dtf/1e0;
    ...
    while (ttot < dtf) {
        ...
        * 
        */
        int rv = cvodes_main_solver( f, jf, input, rtol ,  atol, NSPECIES, (void *) data, dt_now);
...
void dls_read_rate_tables(dls_data *data)
{
    hid_t file_id = H5Fopen("dls_tables.h5", H5F_ACC_RDONLY, H5P_DEFAULT);
...
void dls_read_cooling_tables(dls_data *data)
{
...
void dls_read_gamma(dls_data *data)
{
...
void dls_calculate_temperature(dls_data *data,
...
void dls_interpolate_rates(dls_data *data,
                    int nstrip)
{
...
void dls_interpolate_gamma(dls_data *data,
                    int i)
{   
                                        ...
                                        N_Vector tmp3)
{
    ...
    dls_data *data = (dls_data*)user_data; 
    ...
    dls_calculate_temperature(data, y_arr, nstrip, nchem);

    dls_interpolate_rates(data, nstrip);
...
int calculate_rhs_dls(realtype t, N_Vector y, N_Vector ydot, void *user_data)
{
    dls_data *data = (dls_data* ) user_data;
    int i, j;
