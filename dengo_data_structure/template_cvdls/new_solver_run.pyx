...
cdef extern from "new_solver.h":
    ...
    new_data *new_setup_data(int *NumberOfFields,
            char ***FieldNames)
    void new_read_rate_tables(new_data*)
    void new_read_cooling_tables(new_data*)
    void new_read_gamma(new_data*)

    double dengo_evolve_new (double dtf, double &dt, double z,
    ...
    int calculate_jacobian_new(long int N, realtype t,
                   N_Vector y, N_Vector fy, DlsMat J, void *user_data,
                   N_Vector tmp1, N_Vector tmp2, N_Vector tmp3);

    int calculate_rhs_new(realtype t, N_Vector y, 
...
def run_new(ics, double tf, int niter = 10000,
    ...
    cdef new_data *data = new_setup_data(NULL, NULL)
    cdef rhs_f f = calculate_rhs_new
    cdef jac_f jf = calculate_jacobian_new

    cdef double ttot = 0.0
    cdef int status
    # Allocate some temporary data
    # Now we manually evolve
    #ttot = dengo_evolve_new(tf, dt, input, rtol, atol, dims, data)
    data.current_z = z
