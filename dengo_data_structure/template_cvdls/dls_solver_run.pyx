...
cdef extern from "dls_solver.h":
    ...
    dls_data *dls_setup_data(int *NumberOfFields,
            char ***FieldNames)
    void dls_read_rate_tables(dls_data*)
    void dls_read_cooling_tables(dls_data*)
    void dls_read_gamma(dls_data*)

    double dengo_evolve_dls (double dtf, double &dt, double z,
    ...
    int calculate_jacobian_dls(long int N, realtype t,
                   N_Vector y, N_Vector fy, DlsMat J, void *user_data,
                   N_Vector tmp1, N_Vector tmp2, N_Vector tmp3);

    int calculate_rhs_dls(realtype t, N_Vector y, 
...
def run_dls(ics, double tf, int niter = 10000,
    ...
    cdef dls_data *data = dls_setup_data(NULL, NULL)
    cdef rhs_f f = calculate_rhs_dls
    cdef jac_f jf = calculate_jacobian_dls

    cdef double ttot = 0.0
    cdef int status
    # Allocate some temporary data
    # Now we manually evolve
    #ttot = dengo_evolve_dls(tf, dt, input, rtol, atol, dims, data)
    data.current_z = z
