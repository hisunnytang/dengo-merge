...
cdef extern from "omp_solver.h":
    ...
    omp_data *omp_setup_data(int *NumberOfFields,
            char ***FieldNames)
    void omp_read_rate_tables(omp_data*)
    void omp_read_cooling_tables(omp_data*)
    void omp_read_gamma(omp_data*)

    double dengo_evolve_omp (double dtf, double &dt, double z,
    ...
    int calculate_jacobian_omp(long int N, realtype t,
                   N_Vector y, N_Vector fy, DlsMat J, void *user_data,
                   N_Vector tmp1, N_Vector tmp2, N_Vector tmp3);

    int calculate_rhs_omp(realtype t, N_Vector y, 
...
def run_omp(ics, double tf, int niter = 10000,
    ...
    cdef omp_data *data = omp_setup_data(NULL, NULL)
    cdef rhs_f f = calculate_rhs_omp
    cdef jac_f jf = calculate_jacobian_omp

    cdef double ttot = 0.0
    cdef int status
    # Allocate some temporary data
    # Now we manually evolve
    #ttot = dengo_evolve_omp(tf, dt, input, rtol, atol, dims, data)
    data.current_z = z
