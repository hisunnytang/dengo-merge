...
cdef extern from "temp_omp_solver.h":
    cdef int _MAX_NCELLS  "MAX_NCELLS"
    cdef int _NSPECIES "NSPECIES"

    ctypedef struct temp_omp_data:
        double dbin
        double idbin
        double bounds[2]
        int nbins
    ...
    temp_omp_data *temp_omp_setup_data(int *NumberOfFields,
            char ***FieldNames)
    void temp_omp_read_rate_tables(temp_omp_data*)
    void temp_omp_read_cooling_tables(temp_omp_data*)
    void temp_omp_read_gamma(temp_omp_data*)

    double dengo_evolve_temp_omp (double dtf, double &dt, double z,
    ...
    int calculate_jacobian_temp_omp(long int N, realtype t,
                   N_Vector y, N_Vector fy, DlsMat J, void *user_data,
                   N_Vector tmp1, N_Vector tmp2, N_Vector tmp3);

    int calculate_rhs_temp_omp(realtype t, N_Vector y, 
...
def main_run_temp_omp():
    t1 = time.time()
    temp_omp_main(0, NULL)
    t2 = time.time()
    print "Total elapsed time: %0.3e" % (t2-t1)

def run_temp_omp(ics, double tf, int niter = 10000,
    ...
    cdef temp_omp_data *data = temp_omp_setup_data(NULL, NULL)
    cdef rhs_f f = calculate_rhs_temp_omp
    cdef jac_f jf = calculate_jacobian_temp_omp

    cdef double ttot = 0.0
    cdef int status
    # Allocate some temporary data
    # Now we manually evolve
    #ttot = dengo_evolve_temp_omp(tf, dt, input, rtol, atol, dims, data)
    data.current_z = z
