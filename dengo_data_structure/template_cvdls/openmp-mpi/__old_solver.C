...
#include "__old_solver.h"

__old_data *__old_setup_data(
    int *NumberOfFields, char ***FieldNames)
{
    int i;

    __old_data *data = (__old_data *) malloc(sizeof(__old_data));
    ...
    __old_read_rate_tables(data);
    fprintf(stderr, "Successfully read in rate tables.\n");

    __old_read_cooling_tables(data);
...
{
    __old_data *data = __old_setup_data(NULL, NULL);

    /* Initial conditions */

    hid_t file_id = H5Fopen("__old_initial_conditions.h5", H5F_ACC_RDONLY, H5P_DEFAULT);
    if (file_id < 0) {fprintf(stderr, "Failed to open "
        "__old_initial_conditions.h5 so dying.\n");
        return(1);}
    ...
    double ttot;
    ttot = dengo_evolve___old(dtf, dt, z, input, rtol, atol, dims, data);

    /* Write results to HDF5 file */
    file_id = H5Fcreate("__old_solution.h5", H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
    hsize_t dimsarr[1];
    dimsarr[0] = dims;
    i = 0;
...
double dengo_evolve___old (double dtf, double &dt, double z, double *input,
            double *rtol, double *atol, long long dims, __old_data *data) {
    int i, j;
    hid_t file_id;
    /* fprintf(stderr, "  ncells = % 3i\n", (int) dims); */
    ...
    rhs_f f = calculate_rhs___old;
    jac_f jf = calculate_jacobian___old;
    if (dt < 0) dt = dtf / 1e5;
    data->current_z = z;
    int niter = 0;
    int siter = 0;
...
void __old_read_rate_tables(__old_data *data)
{
    hid_t file_id = H5Fopen("__old_tables.h5", H5F_ACC_RDONLY, H5P_DEFAULT);
...
void __old_read_cooling_tables(__old_data *data)
{
...
void __old_calculate_temperature(__old_data *data,
...
void __old_interpolate_rates(__old_data *data,
                    int nstrip)
{
                  ...
                  int nchem, void *sdata)
{
    ...
    __old_data *data = (__old_data*)sdata;
    int i, j;
    __old_calculate_temperature(data, input, nstrip, nchem);

    __old_interpolate_rates(data, nstrip);
