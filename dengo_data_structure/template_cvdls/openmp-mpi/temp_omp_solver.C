...
void *setup_cvode_solver( rhs_f f, jac_f Jac, double abstol , int , 
        temp_omp_data *data, SUNLinearSolver , SUNMatrix , N_Vector );
int cvode_solver( void *cvode_mem, double *output, int NEQ, double *dt, temp_omp_data * data, N_Vector y);
    ...
    int *NumberOfFields, char ***FieldNames)
{
    ...
    temp_omp_data *data = (temp_omp_data *) malloc(sizeof(temp_omp_data));
...
int temp_omp_main(int argc, char** argv)
{
    temp_omp_data *data = temp_omp_setup_data(NULL, NULL);

    /* Initial conditions */

    hid_t file_id = H5Fopen("temp_omp_initial_conditions.h5", H5F_ACC_RDONLY, H5P_DEFAULT);
    if (file_id < 0) {fprintf(stderr, "Failed to open "
        "temp_omp_initial_conditions.h5 so dying.\n");
        return(1);}
    ...
    double ttot;
    ttot = dengo_evolve_temp_omp(dtf, dt, z, input, rtol, atol, dims, data);

    /* Write results to HDF5 file */
    file_id = H5Fcreate("temp_omp_solution.h5", H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
    hsize_t dimsarr[1];
    dimsarr[0] = dims;
    i = 0;
...
double dengo_evolve_temp_omp (double dtf, double &dt, double z, double *input,
    ...
    rhs_f f = calculate_rhs_temp_omp;
    jac_f jf = calculate_jacobian_temp_omp;
    if (dt < 0) dt = dtf / 1e0;
    data->current_z = z;
    int niter = 0;
    int siter = 0;
...
void temp_omp_read_rate_tables(temp_omp_data *data)
{
    hid_t file_id = H5Fopen("temp_omp_tables.h5", H5F_ACC_RDONLY, H5P_DEFAULT);
...
void temp_omp_read_cooling_tables(temp_omp_data *data)
{
...
void temp_omp_read_gamma(temp_omp_data *data)
{
...
void temp_omp_calculate_temperature(temp_omp_data *data,
                        double *input, int nstrip, int nchem)
{
    ...
    while (Tdiff > 0.1 ){
        ...
        temp_omp_interpolate_gamma(data, i);
...
void temp_omp_interpolate_rates(temp_omp_data *data,
                    int nstrip)
{
...
void temp_omp_interpolate_gamma(temp_omp_data *data,
                    int i)
{   
...
int calculate_jacobian_temp_omp
                                        ( realtype t,
                                        ...
                                        N_Vector tmp3)
{
    ...
    temp_omp_data *data = (temp_omp_data*)user_data; 
    ...
    temp_omp_calculate_temperature(data, y_arr, nstrip, nchem);
    temp_omp_interpolate_rates(data, nstrip);
...
int calculate_rhs_temp_omp(realtype t, N_Vector y, N_Vector ydot, void *user_data)
{
    temp_omp_data *data = (temp_omp_data* ) user_data;
    int i, j;
...
int read_init_data_to_dengo( dengo_field_data *field_data ){
    ...
    char const *filename;
    filename = "temp_omp_initial_conditions.h5";
...
int write_dengo_data_to_file( dengo_field_data *field_data ){
    ...
    hid_t file_id = H5Fcreate("temp_omp_solution.h5", H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
    if (file_id < 0){
        fprintf(stderr, "failed to open %s so dying. \n", "temp_omp_solution.h5");
    }
