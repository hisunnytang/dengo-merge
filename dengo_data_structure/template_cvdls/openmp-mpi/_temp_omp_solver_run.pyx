...
cdef extern from "_temp_omp_solver.h":
    cdef int _MAX_NCELLS  "MAX_NCELLS"
    cdef int _NSPECIES "NSPECIES"

    ctypedef struct _temp_omp_data:
        double dbin
        double idbin
        double bounds[2]
        int nbins
    ...
    _temp_omp_data *_temp_omp_setup_data(int *NumberOfFields,
            char ***FieldNames)
    void _temp_omp_read_rate_tables(_temp_omp_data*)
    void _temp_omp_read_cooling_tables(_temp_omp_data*)
    void _temp_omp_read_gamma(_temp_omp_data*)

    double dengo_evolve__temp_omp (double dtf, double &dt, double z,
    ...
    int calculate_jacobian__temp_omp(long int N, realtype t,
                   N_Vector y, N_Vector fy, DlsMat J, void *user_data,
                   N_Vector tmp1, N_Vector tmp2, N_Vector tmp3);

    int calculate_rhs__temp_omp(realtype t, N_Vector y, 
...
def main_run__temp_omp():
    t1 = time.time()
    _temp_omp_main(0, NULL)
    t2 = time.time()
    print "Total elapsed time: %0.3e" % (t2-t1)

def run__temp_omp(ics, double tf, int niter = 10000,
    ...
    cdef _temp_omp_data *data = _temp_omp_setup_data(NULL, NULL)
    cdef rhs_f f = calculate_rhs__temp_omp
    cdef jac_f jf = calculate_jacobian__temp_omp

    cdef double ttot = 0.0
    cdef int status
    # Allocate some temporary data
    # Now we manually evolve
    #ttot = dengo_evolve__temp_omp(tf, dt, input, rtol, atol, dims, data)
    data.current_z = z
