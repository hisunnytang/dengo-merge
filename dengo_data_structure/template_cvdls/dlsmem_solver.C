    ...
    int *NumberOfFields, char ***FieldNames)
{
    ...
    dlsmem_data *data = (dlsmem_data *) malloc(sizeof(dlsmem_data));
...
{
    dlsmem_data *data = dlsmem_setup_data(NULL, NULL);

    /* Initial conditions */

    hid_t file_id = H5Fopen("dlsmem_initial_conditions.h5", H5F_ACC_RDONLY, H5P_DEFAULT);
    if (file_id < 0) {fprintf(stderr, "Failed to open "
        "dlsmem_initial_conditions.h5 so dying.\n");
        return(1);}
...
void dlsmem_read_rate_tables(dlsmem_data *data)
{
...
void dlsmem_read_cooling_tables(dlsmem_data *data)
{
...
void dlsmem_read_gamma(dlsmem_data *data)
{
...
void dlsmem_calculate_temperature(dlsmem_data *data,
...
void dlsmem_interpolate_rates(dlsmem_data *data,
                    int nstrip)
{
...
void dlsmem_interpolate_gamma(dlsmem_data *data,
                    int i)
{   
