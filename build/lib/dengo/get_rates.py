...
def _get_species(sn):
    ...
    return species_registry[
        "us_%s_%s" % (sn.replace("-", "").replace("+", ""), free_electrons + 1)
    ]
...
def setup_umist_species(species_symbol, atomic_weight):
    ...
    my_sym = MolecularSpecies(
        species_name, atomic_weight, free_electrons, species_symbol
    )


@registry_setup
def setup_umist_reactions(allowed_species):
    umist_names = set(species_registry[s].original_name for s in allowed_species)
    # umist_names.add("PHOTON")
    fn = os.path.join(os.path.dirname(__file__), "RATE12.txt")
    for line in open(fn, "r"):
        rline = re.split(":?", line)
        if not all(r in umist_names for r in rline[2:6]):
            continue
