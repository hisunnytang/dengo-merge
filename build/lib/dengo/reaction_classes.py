...
def registry_setup(func):
    def _wfunc(*args, **kwargs):
        old_names = [
            set(d.keys())
            for d in (species_registry, cooling_registry, reaction_registry)
        ]
        func(*args, **kwargs)
        nn = []
        for on, r in zip(
            old_names, (species_registry, cooling_registry, reaction_registry)
        ):
...
def ion_photoionization_rate(species, photo_background="HM12"):
    ...
    def photoionization_rate(network):
        ...
        f = h5py.File(
            "input/photoionization/%s_ion_by_ion_photoionization_%s.h5"
        ...
        if end_method == 0:
            ...
            # extrapolate
            extrapdown = logdataS[0] + (logz - logdataz[0]) * (
                logdataS[0] - logdataS[1]
            ) / (logdataz[0] - logdataz[1])
            vals[logz < logdataz[0]] = extrapdown[logz < logdataz[0]]
            extrapup = logdataS[-1] + (logz - logdataz[-1]) * (
                logdataS[-1] - logdataS[-2]
        ...
        if end_method == 1:
            ...
            gaussdown = (
                dataS[0]
                * (tiny / dataS[0]) ** (((z - dataz[0]) / (z[0] - dataz[0]))) ** 2
            )
            vals[z < dataz[0]] = gaussdown[z < dataz[0]]
            gaussup = (
                dataS[-1]
                * (tiny / dataS[-1]) ** (((z - dataz[-1]) / (z[-1] - dataz[-1]))) ** 2
            )
...
class ChemicalSpecies(Species):
    def __init__(self, name, weight, free_electrons=0.0, pretty_name=None):
        self.weight = weight
...
class AtomicSpecies(ChemicalSpecies):
    def __init__(self, atom_name, free_electrons):
        ...
        pretty_name = "%s with %s free electrons" % (pn, free_electrons)
        super(AtomicSpecies, self).__init__(name, weight, free_electrons, pretty_name)


class MolecularSpecies(ChemicalSpecies):
    def __init__(self, molecule_name, weight, free_electrons, original_name=None):
        name = "%s_%i" % (molecule_name, free_electrons + 1)
        pretty_name = "%s with %s free electrons" % (name, free_electrons)
        self.original_name = original_name or molecule_name
        super(MolecularSpecies, self).__init__(
            name, weight, free_electrons, pretty_name
        )
...
class CoolingAction(object):
    ...
    def equation(self):
        ...
        ta_sym = dict(
            (n, sympy.Symbol("%s_%s[i]" % (self.name, n))) for n in self.tables
        )
...
def ion_cooling_rate(species, atom_name):
    ...
    def cooling_rate(network):
        ...
        if end_method == 0:
            ...
            # extrapolate
            extrapdown = logdataS[0] + (logT - logdataT[0]) * (
                logdataS[0] - logdataS[1]
            ) / (logdataT[0] - logdataT[1])
            vals[logT < logdataT[0]] = extrapdown[logT < logdataT[0]]
            extrapup = logdataS[-1] + (logT - logdataT[-1]) * (
                logdataS[-1] - logdataS[-2]
        ...
        if end_method == 1:
            ...
            gaussdown = (
                dataS[0]
                * (tiny / dataS[0]) ** (((T - dataT[0]) / (T[0] - dataT[0]))) ** 2
            )
            vals[T < dataT[0]] = gaussdown[T < dataT[0]]
            gaussup = (
                dataS[-1]
                * (tiny / dataS[-1]) ** (((T - dataT[-1]) / (T[-1] - dataT[-1]))) ** 2
            )
    ...
    ion_cooling_action = CoolingAction(
        "%s_c" % species.name, "-%s_c * %s * de" % (species.name, species.name)  # name
    )  # equation
...
def ion_photoheating_rate(species, photo_background="HM12"):
    ...
    def photoheating_rate(network):
        ...
        # NOTE: these rates do the interpolation as a function fo redshift
        f = h5py.File(
            "input/photoheating/%s_ion_by_ion_photoheating_%s.h5"
            % (element_name, photo_background)
        )
    ...
    if species_ph in species_registry:
        ...
        ion_cooling_action = CoolingAction(
            "%s_ph" % species.name, "%s_ph * %s" % (species.name, species.name)  # name
        )  # equation
