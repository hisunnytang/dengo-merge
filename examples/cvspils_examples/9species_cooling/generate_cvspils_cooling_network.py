...
def Init_values(temperature, density, n_species = 6, cooling=True):
...
def create_cvspils_solver(init, primordial, solver_name, cooling):
    ...
    sundials_cvspils_run = pyximport.load_module("{}_run".format(solver_name),
                    "{}_solver_run.pyx".format(solver_name),
                     build_inplace = True, pyxbuild_dir = "_dengo_temp")
    return sundials_cvspils_run


def solver_performance(Tdim = 50 ,Ddim = 50 , n_species=9, solver_name = "cvspils_9species", cooling=False):
    time_taken = []
    success = []
    ...
    chemistry_run = create_cvspils_solver(init, primordial, solver_name, cooling=cooling)
