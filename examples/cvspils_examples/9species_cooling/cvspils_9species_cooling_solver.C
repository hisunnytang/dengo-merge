...
#include "cvspils_9species_cooling_solver.h"

cvspils_9species_cooling_data *cvspils_9species_cooling_setup_data(
    int *NumberOfFields, char ***FieldNames)
{
    int i;

    cvspils_9species_cooling_data *data = (cvspils_9species_cooling_data *) malloc(sizeof(cvspils_9species_cooling_data));
    ...
    cvspils_9species_cooling_read_rate_tables(data);
    fprintf(stderr, "Successfully read in rate tables.\n");

    cvspils_9species_cooling_read_cooling_tables(data);
...
int cvspils_9species_cooling_main(int argc, char** argv)
{
    cvspils_9species_cooling_data *data = cvspils_9species_cooling_setup_data(NULL, NULL);

    /* Initial conditions */

    hid_t file_id = H5Fopen("cvspils_9species_cooling_initial_conditions.h5", H5F_ACC_RDONLY, H5P_DEFAULT);
    if (file_id < 0) {fprintf(stderr, "Failed to open "
        "cvspils_9species_cooling_initial_conditions.h5 so dying.\n");
        return(1);}
    ...
    double ttot;
    ttot = dengo_evolve_cvspils_9species_cooling(dtf, dt, z, input, rtol, atol, dims, data);

    /* Write results to HDF5 file */
    file_id = H5Fcreate("cvspils_9species_cooling_solution.h5", H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
    hsize_t dimsarr[1];
    dimsarr[0] = dims;
    i = 0;
...
double dengo_evolve_cvspils_9species_cooling (double dtf, double &dt, double z, double *input,
            double *rtol, double *atol, long long dims, cvspils_9species_cooling_data *data) {
    int i, j;
    hid_t file_id;
    /* fprintf(stderr, "  ncells = % 3i\n", (int) dims); */
    ...
    rhs_f f = calculate_rhs_cvspils_9species_cooling;
    jac_f jf = calculate_JacTimesVec_cvspils_9species_cooling;
    if (dt < 0) dt = dtf / 1e5;
    data->current_z = z;
    int niter = 0;
    int siter = 0;
...
void cvspils_9species_cooling_read_rate_tables(cvspils_9species_cooling_data *data)
{
    hid_t file_id = H5Fopen("cvspils_9species_cooling_tables.h5", H5F_ACC_RDONLY, H5P_DEFAULT);
...
void cvspils_9species_cooling_read_cooling_tables(cvspils_9species_cooling_data *data)
{
...
void cvspils_9species_cooling_calculate_temperature(cvspils_9species_cooling_data *data,
...
void cvspils_9species_cooling_interpolate_rates(cvspils_9species_cooling_data *data,
                    int nstrip)
{
...
int calculate_JacTimesVec_cvspils_9species_cooling
             ...
             void *user_data, N_Vector tmp)
{
    ...
    int nstrip = 1;
    int nchem = 10;
    cvspils_9species_cooling_data *data = (cvspils_9species_cooling_data*)user_data; 
    ...
    cvspils_9species_cooling_calculate_temperature(data, y_arr, nstrip, nchem);
    cvspils_9species_cooling_interpolate_rates(data, nstrip);
    ...
    for (i = 0; i<nstrip; i++) {
        ...
        // Species: H2_1
        //
        Ith(Jv, 1 ) = -k11[i]*H2_1*v9 - k12[i]*H2_1*v8 + v0*(-k11[i]*H_2 - k12[i]*de - k13[i]*H_1 + k21[i]*pow(H_1, 2) - 2*k23[i]*H2_1) + v1*(k10[i]*H_1 + k19[i]*H_m0) + v4*(k08[i]*H_1 + k19[i]*H2_2) + v7*(k08[i]*H_m0 + k10[i]*H2_2 - k13[i]*H2_1 + 2*k21[i]*H2_1*H_1 + 3*k22[i]*pow(H_1, 2));
...
int calculate_rhs_cvspils_9species_cooling(realtype t, N_Vector y, N_Vector ydot, void *user_data)
{
    cvspils_9species_cooling_data *data = (cvspils_9species_cooling_data* ) user_data;
    int i, j;
