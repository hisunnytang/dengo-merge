...
# filename: sundials_CVSpils_solver_run.pyx
...
cdef extern from "cvspils_9species_cooling_solver.h":
    cdef int _MAX_NCELLS  "MAX_NCELLS"
    cdef int _NSPECIES "NSPECIES"

    ctypedef struct cvspils_9species_cooling_data:
        double dbin
        double idbin
        double bounds[2]
        int nbins
    ...
    ctypedef int(*jac_f)(N_Vector , N_Vector , realtype,
             N_Vector, N_Vector,
             void *user_data, N_Vector)
 
    int cvspils_9species_cooling_main(int argc, char **argv)
    cvspils_9species_cooling_data *cvspils_9species_cooling_setup_data(int *NumberOfFields,
            char ***FieldNames)
    void cvspils_9species_cooling_read_rate_tables(cvspils_9species_cooling_data*)
    void cvspils_9species_cooling_read_cooling_tables(cvspils_9species_cooling_data*)
    double dengo_evolve_cvspils_9species_cooling (double dtf, double &dt, double z,
                                         double *input, double *rtol,
                                         double *atol, int dims,
                                         cvspils_9species_cooling_data *data);

    # Declare the Jacobian and RHS function
    
    int calculate_JacTimesVec_cvspils_9species_cooling(N_Vector , N_Vector , realtype, N_Vector, N_Vector, void *user_data, N_Vector);

    int calculate_rhs_cvspils_9species_cooling(realtype t, N_Vector y, 
...
#cdef extern from "cvodes_solver_CVSpils.C":
#    int cvodes_main_solver( rhs_f f, jac_f jf, 
#                 double *input, double *rtol, double *atol,
#                 int nchem, void *sdata, double *dt_now);


def main_run_cvspils_9species_cooling():
    t1 = time.time()
    cvspils_9species_cooling_main(0, NULL)
    t2 = time.time()
    print "Total elapsed time: %0.3e" % (t2-t1)

def run_cvspils_9species_cooling(ics, double tf, int niter = 10000,
    ...
    cdef cvspils_9species_cooling_data *data = cvspils_9species_cooling_setup_data(NULL, NULL)
    cdef rhs_f f = calculate_rhs_cvspils_9species_cooling
    cdef jac_f jf = calculate_JacTimesVec_cvspils_9species_cooling

    cdef double ttot = 0.0
    cdef int status
    # Allocate some temporary data
    # Now we manually evolve
    #ttot = dengo_evolve_cvspils_9species_cooling(tf, dt, input, rtol, atol, dims, data)
    data.current_z = z
