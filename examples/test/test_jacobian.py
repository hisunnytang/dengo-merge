...
class TestChemicalNetworkSolution:
    ...
    def cooling_term_solution(self, ca):
        if ca == "h2formation":
            eqH2formation = (
                0.5
                * (ncrn / (ncrd1 * H1 + ncrd2 * H2) + 1.0) ** -1.0
    ...
    def jacobian_solution(self, s1, s2):
        if s1.name == "ge" and s2.name == "ge":
            ...
            d1dge = 0.5 * (ncrn / (ncrd1 * H1 + ncrd2 * H2) + 1.0) ** (-2.0) * (
                ...
                * (-H2 * rncrd2 - H1 * rncrd1)
                / (ncrd1 * H1 + ncrd2 * H2) ** 2
                - 1.0 * rncrn / (ncrd1 * H1 + ncrd2 * H2)
            ) + 0.5 * (
                ncrn / (ncrd1 * H1 + ncrd2 * H2) + 1.0
            ) ** (
                -1.0
            ) * (
            ...
            # gloverable08
            d2dge = -h2lte * H2 * h2opticaldepth * (
                -h2lte
                * (-H2 * rgaH2 - H1 * rgaHI - H_2 * rgaHp - He_1 * rgaHe - de * rgael)
                / (gaH2 * H2 + gaHI * H1 + gaHe * He_1 + gaHp * H_2 + gael * de) ** 2
                - rh2lte
                / (gaH2 * H2 + gaHI * H1 + gaHe * He_1 + gaHp * H_2 + gael * de)
            ) / (
                h2lte / (gaH2 * H2 + gaHI * H1 + gaHe * He_1 + gaHp * H_2 + gael * de)
                + 1.0
            ) ** 2 - H2 * h2opticaldepth * rh2lte / (
                h2lte / (gaH2 * H2 + gaHI * H1 + gaHe * He_1 + gaHp * H_2 + gael * de)
                + 1.0
            )
            return d1dge + d2dge
