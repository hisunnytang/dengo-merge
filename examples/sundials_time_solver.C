...
sundials_time_data *sundials_time_setup_data(
    int *NumberOfFields, char ***FieldNames)
{
    int i;

    sundials_time_data *data = (sundials_time_data *) malloc(sizeof(sundials_time_data));
    ...
    sundials_time_read_rate_tables(data);
    fprintf(stderr, "Successfully read in rate tables.\n");

    sundials_time_read_cooling_tables(data);
...
{
    sundials_time_data *data = sundials_time_setup_data(NULL, NULL);

    /* Initial conditions */

    hid_t file_id = H5Fopen("sundials_time_initial_conditions.h5", H5F_ACC_RDONLY, H5P_DEFAULT);
    if (file_id < 0) {fprintf(stderr, "Failed to open "
        "sundials_time_initial_conditions.h5 so dying.\n");
        return(1);}
    ...
    double ttot;
    ttot = dengo_evolve_sundials_time(dtf, dt, z, input, rtol, atol, dims, data);

    /* Write results to HDF5 file */
    file_id = H5Fcreate("sundials_time_solution.h5", H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
    hsize_t dimsarr[1];
    dimsarr[0] = dims;
    i = 0;
...
double dengo_evolve_sundials_time (double dtf, double &dt, double z, double *input,
            double *rtol, double *atol, long long dims, sundials_time_data *data) {
    int i, j;
    hid_t file_id;
    /* fprintf(stderr, "  ncells = % 3i\n", (int) dims); */
    ...
    rhs_f f = calculate_rhs_sundials_time;
    jac_f jf = calculate_jacobian_sundials_time;
    if (dt < 0) dt = dtf / 1e5;
    data->current_z = z;
    int niter = 0;
    int siter = 0;
...
void sundials_time_read_rate_tables(sundials_time_data *data)
{
    hid_t file_id = H5Fopen("sundials_time_tables.h5", H5F_ACC_RDONLY, H5P_DEFAULT);
...
void sundials_time_read_cooling_tables(sundials_time_data *data)
{
...
void sundials_time_calculate_temperature(sundials_time_data *data,
...
void sundials_time_interpolate_rates(sundials_time_data *data,
                    int nstrip)
{
...
int calculate_jacobian_sundials_time
                                        ...
                                        N_Vector tmp3)
{
    ...
    sundials_time_data *data = (sundials_time_data*)user_data; 
    ...
    sundials_time_calculate_temperature(data, y_arr, nstrip, nchem);

    sundials_time_interpolate_rates(data, nstrip);
...
int calculate_rhs_sundials_time(realtype t, N_Vector y, N_Vector ydot, void *user_data)
{
    sundials_time_data *data = (sundials_time_data* ) user_data;
    int i, j;
