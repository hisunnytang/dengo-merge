...
cdef extern from "cvspils_9species_solver.h":
    cdef int _MAX_NCELLS  "MAX_NCELLS"
    cdef int _NSPECIES "NSPECIES"

    ctypedef struct cvspils_9species_data:
        double dbin
        double idbin
        double bounds[2]
        int nbins
    ...
    cvspils_9species_data *cvspils_9species_setup_data(int *NumberOfFields,
            char ***FieldNames)
    void cvspils_9species_read_rate_tables(cvspils_9species_data*)
    void cvspils_9species_read_cooling_tables(cvspils_9species_data*)
    double dengo_evolve_cvspils_9species (double dtf, double &dt, double z,
                                         double *input, double *rtol,
                                         double *atol, int dims,
                                         cvspils_9species_data *data);

    # Declare the Jacobian and RHS function

    int calculate_JacTimesVec_cvspils_9species(N_Vector , N_Vector , realtype, N_Vector, N_Vector, void *user_data, N_Vector);

    int calculate_rhs_cvspils_9species(realtype t, N_Vector y,
...
def main_run_cvspils_9species():
    t1 = time.time()
    cvspils_9species_main(0, NULL)
    t2 = time.time()
    print "Total elapsed time: %0.3e" % (t2-t1)

def run_cvspils_9species(ics, double tf, int niter = 10000,
    ...
    cdef cvspils_9species_data *data = cvspils_9species_setup_data(NULL, NULL)
    cdef rhs_f f = calculate_rhs_cvspils_9species
    cdef jac_f jf = calculate_JacTimesVec_cvspils_9species

    cdef double ttot = 0.0
    cdef int status
    # Allocate some temporary data
    # Now we manually evolve
    #ttot = dengo_evolve_cvspils_9species(tf, dt, input, rtol, atol, dims, data)
    data.current_z = z
