...
cvspils_9species_data *cvspils_9species_setup_data(
    int *NumberOfFields, char ***FieldNames)
{
    ...
    cvspils_9species_read_cooling_tables(data);
...
int cvspils_9species_main(int argc, char** argv)
{
    cvspils_9species_data *data = cvspils_9species_setup_data(NULL, NULL);

    /* Initial conditions */

    hid_t file_id = H5Fopen("cvspils_9species_initial_conditions.h5", H5F_ACC_RDONLY, H5P_DEFAULT);
    if (file_id < 0) {fprintf(stderr, "Failed to open "
        "cvspils_9species_initial_conditions.h5 so dying.\n");
        return(1);}
    ...
    double ttot;
    ttot = dengo_evolve_cvspils_9species(dtf, dt, z, input, rtol, atol, dims, data);

    /* Write results to HDF5 file */
    file_id = H5Fcreate("cvspils_9species_solution.h5", H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
    hsize_t dimsarr[1];
    dimsarr[0] = dims;
    i = 0;
...
double dengo_evolve_cvspils_9species (double dtf, double &dt, double z, double *input,
            double *rtol, double *atol, long long dims, cvspils_9species_data *data) {
    int i, j;
    hid_t file_id;
    /* fprintf(stderr, "  ncells = % 3i\n", (int) dims); */
    ...
    rhs_f f = calculate_rhs_cvspils_9species;
    jac_f jf = calculate_JacTimesVec_cvspils_9species;
    if (dt < 0) dt = dtf / 1e5;
    data->current_z = z;
    int niter = 0;
    int siter = 0;
...
{
    hid_t file_id = H5Fopen("cvspils_9species_tables.h5", H5F_ACC_RDONLY, H5P_DEFAULT);
...
void cvspils_9species_read_cooling_tables(cvspils_9species_data *data)
{
...
void cvspils_9species_interpolate_rates(cvspils_9species_data *data,
                    int nstrip)
{
...
int calculate_rhs_cvspils_9species(realtype t, N_Vector y, N_Vector ydot, void *user_data)
{
    cvspils_9species_data *data = (cvspils_9species_data* ) user_data;
    int i, j;
