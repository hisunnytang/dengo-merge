...
from dengo.oxygen_cooling import cooling_action_table, cooling_rates_table
...
for rxn_name in na.sort(list(oxygen_network.reactions.keys())):
    ...
    dset.attrs.create(
        "reaction_equation", data=oxygen_network.reactions[rxn_name].__str__()
    )
f.close()
...
for r in cooling_rate_list:
    ...
    dset = f.create_dataset(
        "/%s" % r, data=cooling_rates_table["%s" % r].values.astype("float64")
    )
f.close()
