...
cdef extern from "cvdls_9species_solver.h":
    cdef int _MAX_NCELLS  "MAX_NCELLS"
    cdef int _NSPECIES "NSPECIES"

    ctypedef struct cvdls_9species_data:
        double dbin
        double idbin
        double bounds[2]
        int nbins
    ...
    cvdls_9species_data *cvdls_9species_setup_data(int *NumberOfFields,
            char ***FieldNames)
...
def main_run_cvdls_9species():
    t1 = time.time()
    cvdls_9species_main(0, NULL)
    t2 = time.time()
    print "Total elapsed time: %0.3e" % (t2-t1)

def run_cvdls_9species(ics, double tf, int niter = 10000,
    ...
    #ttot = dengo_evolve_cvdls_9species(tf, dt, input, rtol, atol, dims, data)
    data.current_z = z
