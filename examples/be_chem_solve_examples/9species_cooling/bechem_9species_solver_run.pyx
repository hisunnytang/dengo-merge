...
cdef extern from "bechem_9species_solver.h":
    cdef int _MAX_NCELLS  "MAX_NCELLS"
    cdef int _NSPECIES "NSPECIES"
    ctypedef struct bechem_9species_data:
        double dbin
        double idbin
        double bounds[2]
        int nbins
    ...
    bechem_9species_data *bechem_9species_setup_data(int *NumberOfFields,
            char ***FieldNames)
    void bechem_9species_read_rate_tables(bechem_9species_data*)
    void bechem_9species_read_cooling_tables(bechem_9species_data*)
    double dengo_evolve_bechem_9species (double dtf, double &dt, double z,
                                         double *input, double *rtol,
                                         double *atol, int dims,
                                         bechem_9species_data *data)
...
def main_run_bechem_9species():
    t1 = time.time()
    bechem_9species_main(0, NULL)
    t2 = time.time()
    print "Total elapsed time: %0.3e" % (t2-t1)

def run_bechem_9species(ics, double tf, int niter = 10000,
    ...
    cdef bechem_9species_data *data = bechem_9species_setup_data(NULL, NULL)
    cdef rhs_f f = calculate_rhs_bechem_9species
    cdef jac_f jf = calculate_jacobian_bechem_9species
    ...
    #ttot = dengo_evolve_bechem_9species(tf, dt, input, rtol, atol, dims, data)
    data.current_z = z
