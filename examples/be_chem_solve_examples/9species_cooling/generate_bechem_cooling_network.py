...
def Init_values(temperature, density, n_species=9, cooling=True):
    ...
    if n_species == 6:
        ...
        init_values["H_2"] = density * (0.76) / 2.0
    else:
        # 9-species model
        init_values["He_1"] = density * (1.0 - 0.76) / 2.0
        init_values["He_2"] = density * (1.0 - 0.76) / 2.0
        init_values["He_3"] = np.array([tiny])
        init_values["H_1"] = density * (0.76) / 3.0
        init_values["H_2"] = density * (0.76) / 3.0

        init_values["H_m0"] = np.array([tiny])
        init_values["H2_1"] = density * (0.76) / 3.0
    ...
    ge = (temperature * kboltz) * gamma_factor / (init_values["density"] * mh)
...
def create_bechem_solver(init, primordial, solver_name, cooling):
    ...
    bechem_run = pyximport.load_module(
        "{}_run".format(solver_name),
...
def solver_performance(
    Tdim=50, Ddim=50, n_species=9, solver_name="bechem_9species", cooling=True
):
    time_taken = []
    success = []
    ...
    init, primordial = Init_values(
        np.array([1000]), np.array([1e10]), n_species=n_species
    )
    ...
    for temp, den in den_temp_pair2d:
        ...
        init, primordial = Init_values(
            np.array([temp]), np.array([den]), n_species=n_species
        )
        total_t = calc_fftime(den).v
        # Calculate the time it takes
        tic = timeit.default_timer()
        rv, rv_int = eval(
            "chemistry_run.run_" + solver_name + "(init, total_t, niter=1e4)"
        )
...
if __name__ == "__main__":
    ...
    chemistry_run = create_bechem_solver(init, primordial, solver_name, cooling=True)
    success, time_taken, temp_arr, den_arr, filename = solver_performance(
        Tdim=20, Ddim=20, n_species=9, solver_name="bechem_9species", cooling=True
    )
