...
void *setup_cvode_solver( rhs_f f, jac_f Jac,  int NEQ, 
        cache_data *data, SUNLinearSolver LS, SUNMatrix A, N_Vector y, double reltol, N_Vector abstol);

int cvode_solver( void *cvode_mem, double *output, int NEQ, double *dt, cache_data * data, N_Vector y, double reltol, N_Vector abstol );

cache_data *cache_setup_data(int *, char***);
void cache_read_rate_tables(cache_data*);
void cache_read_cooling_tables(cache_data*);
void cache_read_gamma(cache_data*);
void cache_interpolate_gamma(cache_data*, int );
...
int cache_calculate_temperature(cache_data *data, double *input, int nstrip, int nchem);
