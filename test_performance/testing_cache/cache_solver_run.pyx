...
cdef extern from "cache_solver.h":
    ...
    cache_data *cache_setup_data(int *NumberOfFields,
            char ***FieldNames)
    void cache_read_rate_tables(cache_data*)
    void cache_read_cooling_tables(cache_data*)
    void cache_read_gamma(cache_data*)

    double dengo_evolve_cache (double dtf, double &dt, double z,
                                         double *input, double *rtol,
                                         double *atol, int dims,
                                         cache_data *data, double *temp)

    # Declare the Jacobian and RHS function
    
    int calculate_jacobian_cache(long int N, realtype t,
                   N_Vector y, N_Vector fy, DlsMat J, void *user_data,
                   N_Vector tmp1, N_Vector tmp2, N_Vector tmp3);

    int calculate_rhs_cache(realtype t, N_Vector y, 
                    N_Vector ydot, void *user_data);
    
    void cache_calculate_temperature(cache_data *data, double *input, int nstrip, int nchem);
...
def run_cache(ics, double tf, int niter = 10000,
    ...
    cdef cache_data *data = cache_setup_data(NULL, NULL)
    cdef rhs_f f = calculate_rhs_cache
    cdef jac_f jf = calculate_jacobian_cache

    cdef double ttot = 0.0
    cdef int status
    # Allocate some temporary data
    # Now we manually evolve
    #ttot = dengo_evolve_cache(tf, dt, input, rtol, atol, dims, data)
    data.current_z = z
    ...
    for iter in range(niter):
        dt_local = dengo_evolve_cache( dt0[0], dt[0], z, input, rtol, atol, dims, data, temp  )
        if dt_local > 0:
            status = 0
