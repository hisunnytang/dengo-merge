...
def setup_primordial():
    ...
    def rxn(state):
        ...
        vals = 3.0e-16 * ( state.T/3.e2)**0.95 \
                * np.exp(-state.T/9.32e3)
...
#        vals[_i2] = 1.43e-9
        ...
        #
        vals = 1.35e-9*(state.T**9.8493e-2 + 3.2852e-1 \
         * state.T**5.5610e-1 + 2.771e-7 * state.T**2.1826)\
        / (1. + 6.191e-3 * state.T**1.0461\
    ...
    def rxn(state):
        ...
        if state.threebody == 0:
            ...
            vals[_i2] = 2.2e-6 * state.T[_i2] **(-0.565) * np.exp( -5.2e4/ state.T[_i2])
