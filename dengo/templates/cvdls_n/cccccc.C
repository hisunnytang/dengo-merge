    ...
    */
    ...
    for (int d = 0; d < dims; d++){
        ...
        for ( i = 0; i < N; i++){
            ...
            while (ttot < dtf) {
                /* f and jf are function for evaluating 
                * rhs and jac for the solver
                * input: {double array}
	    ...
	    for (i = 0; i < N; i++) prev_N[i] = input_N[i];
            ...
            dt /= 2.0;
            for (i = 0; i < dims * N; i++) input_N[i] = prev_N[i];
            if (dt/dtf < 1e-15)  {
                                        ...
                                        N_Vector tmp3)
{
    ...
    for (i = 0; i<nstrip; i++) {
        ...
        scale2 = data->scale[{{i_s2.index - 1}}];
        scale1 = data->scale[{{i_s1.index - 1}}];
        IJth(J, {{i_s2.index}}, {{i_s1.index}}) /= scale2/scale1;

        {% if s2.name == 'ge' %}
        IJth(J, {{i_s2.index}}, {{i_s1.index}} ) /= mdensity;
        {% endif %}
...
{{solver_name}}_data *data, double dt ){
    ...
    ttot = dengo_evolve_{{solver_name}}( dt, dt, z, input, rtol, atol, dims, data );
    ...
    for ( int d = 0; d< dims; d++  ){
        ...
        {%- for species in network.required_species | sort %}
        field_Data->{{species.name}}_density[d] = input[j] * {{species.weight}};
        j++;
        {% endfor %}
    }
...
int read_init_data_to_dengo( dengo_field_data *field_data ){
    ...
    {%- for species in network.required_species | sort %}
    double *{{species_name}} = (double *) malloc(dims * sizeof(double));    
    field_data->{{species.name}} = {{species.name}};
    {% endfor %}
