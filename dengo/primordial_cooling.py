...
from .chemistry_constants import kboltz, tevk
...
def cool(eq):
    ...
    def ceHI(state):
        vals = (
            7.5e-19
            * np.exp(-np.minimum(np.log(dhuge), 118348.0 / state.T))
...
def cool(eq):
    ...
    def ceHeI(state):
        vals = (
            9.1e-27
            * np.exp(-np.minimum(np.log(dhuge), 13179.0 / state.T))
            * state.T ** (-0.1687)
            / (1.0 + np.sqrt(state.T / 1.0e5))
        )
        return vals
...
def cool(eq):
    ...
    def ceHeII(state):
        vals = (
            5.54e-17
            * np.exp(-np.minimum(np.log(dhuge), 473638.0 / state.T))
            * state.T ** (-0.397)
            / (1.0 + np.sqrt(state.T / 1.0e5))
        )
        return vals
...
def cool(eq):
    ...
    def reHeII2(state):
        vals = (
            ...
            * (1.0 + 0.3 * np.exp(-np.minimum(np.log(dhuge), 94000.0 / state.T)))
        )
        return vals
...
@cooling_action(
    "gloverabel08", "- h2_optical_depth_approx *  (H2I)*h2lte/(1.0+h2lte/galdl) "
)
def cool(eq):
    @eq.table
    def gpldl(state):
...
def h2formation(eq):
    ...
    eq.temporary(
        "h2heatfrac", " (1.0 + ncrn / (HI * ncrd1 + H2I * ncrd2))**(-1.0)/2.0 "
    )


@cooling_action(
    "h2formation_extra", "h2heatfrac*(h2mheat_extra*HI*HI*H2I - h2mcool_extra*H2I*H2I)"
)
def h2formation(eq):
    @eq.table
