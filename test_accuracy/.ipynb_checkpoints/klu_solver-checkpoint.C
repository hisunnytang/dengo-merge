...
klu_data *klu_setup_data(
    int *NumberOfFields, char ***FieldNames)
{
    int i;

    klu_data *data = (klu_data *) malloc(sizeof(klu_data));
    ...
    klu_read_rate_tables(data);
    fprintf(stderr, "Successfully read in rate tables.\n");

    klu_read_cooling_tables(data);
...
{
    klu_data *data = klu_setup_data(NULL, NULL);

    /* Initial conditions */
    hid_t file_id;
    if (argc < 2){
    file_id = H5Fopen("klu_initial_conditions.h5", H5F_ACC_RDONLY, H5P_DEFAULT);
    if (file_id < 0) {fprintf(stderr, "Failed to open "
        "klu_initial_conditions.h5 so dying.\n");
        return(1);}
    } else {
    ...
    file_id = H5Fopen("klu_initial_conditions.h5", H5F_ACC_RDONLY, H5P_DEFAULT);
    status = H5LTget_dataset_info(file_id, "/ge", &dims, NULL, NULL);
    
    if (argc < 3){
        file_id = H5Fcreate("klu_solution.h5", H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
    } else{
...
double dengo_evolve_klu (double dtf, double &dt, double z, double *input,
    ...
    rhs_f f = calculate_rhs_klu;

    #ifndef CVSPILS
    #ifdef  CVKLU
    jac_f jf = calculate_sparse_jacobian_klu;
    #else
    jac_f jf = calculate_jacobian_klu;
    #endif
    #endif
    
    #ifdef CVSPILS
    jac_f jf = calculate_JacTimesVec_klu;
    #endif
    ...
    for (int d = 0; d < ntimes; d++){
        ...
        setting_up_extra_variables(data, data->scale, dims);
        ...
        klu_calculate_temperature(data, data->scale, nstrip, N);
...
void klu_read_rate_tables(klu_data *data)
{
    hid_t file_id = H5Fopen("klu_tables.h5", H5F_ACC_RDONLY, H5P_DEFAULT);
...
void klu_read_cooling_tables(klu_data *data)
{
...
void klu_read_gamma(klu_data *data)
{
...
int klu_calculate_temperature(klu_data *data,
...
void klu_interpolate_rates(klu_data *data,
                    int nstrip)
{
...
void klu_interpolate_gamma(klu_data *data,
                    int i)
{   
                                        ...
                                        N_Vector tmp3)
{
    ...
    klu_data *data = (klu_data*)user_data; 
    ...
    int flag;
    flag = klu_calculate_temperature(data, y_arr , nstrip, nchem );
    if (flag > 0){
        // check if the temperature failed to converged
        return -1;    
    }
    klu_interpolate_rates(data, nstrip);
    */

    // klu_calculate_temperature(data, y_arr, nstrip, nchem);
    // klu_interpolate_rates(data, nstrip);
    ...
    for ( i = 0; i < nstrip; i++ ){
        ...
        SM_ELEMENT_D( J, j + 9, j + 0 ) *= data->cie_optical_depth_approx[i]; 
        ...
        SM_ELEMENT_D( J, j + 9, j + 1 ) *= data->cie_optical_depth_approx[i]; 
        ...
        SM_ELEMENT_D( J, j + 9, j + 2 ) *= data->cie_optical_depth_approx[i]; 
        ...
        SM_ELEMENT_D( J, j + 9, j + 3 ) *= data->cie_optical_depth_approx[i]; 
        ...
        SM_ELEMENT_D( J, j + 9, j + 4 ) *= data->cie_optical_depth_approx[i]; 
        ...
        SM_ELEMENT_D( J, j + 9, j + 5 ) = (-H2_1*gloverabel08_gaHe[i]*pow(gloverabel08_h2lte[i], 2)*h2_optical_depth_approx/(pow(gloverabel08_h2lte[i]/(H2_1*gloverabel08_gaH2[i] + H_1*gloverabel08_gaHI[i] + H_2*gloverabel08_gaHp[i] + He_1*gloverabel08_gaHe[i] + de*gloverabel08_gael[i]) + 1.0, 2)*pow(H2_1*gloverabel08_gaH2[i] + H_1*gloverabel08_gaHI[i] + H_2*gloverabel08_gaHp[i] + He_1*gloverabel08_gaHe[i] + de*gloverabel08_gael[i], 2)) - ciHeI_ciHeI[i]*de)*fmin(1.00000000000000, (1.0 - exp(-fmax(1.00000000000000e-5, 8.87484891418606e+20*pow(mdensity, 2.8))))/fmax(1.00000000000000e-5, 8.87484891418606e+20*pow(mdensity, 2.8)));
        inv_scale2 = inv_scale[ j + 9];
        scale1     = scale    [ j + 5];
        ...
        SM_ELEMENT_D( J, j + 9, j + 5 ) *= data->cie_optical_depth_approx[i]; 
        ...
        SM_ELEMENT_D( J, j + 9, j + 6 ) *= data->cie_optical_depth_approx[i]; 
        ...
        SM_ELEMENT_D( J, j + 9, j + 7 ) *= data->cie_optical_depth_approx[i]; 
        ...
        SM_ELEMENT_D( J, j + 9, j + 8 ) *= data->cie_optical_depth_approx[i]; 
        ...
        SM_ELEMENT_D( J, j + 9, j + 9 ) *= data->cie_optical_depth_approx[i]; 
...
int calculate_rhs_klu(realtype t, N_Vector y, N_Vector ydot, void *user_data)
{
    ...
    for ( i = 0; i < nstrip; i++ ){
        ...
        // Species: ge
        //
        NV_Ith_S( ydot, j ) = (-2.01588*H2_1*cie_cooling_cieco[i]*mdensity - H2_1*gloverabel08_h2lte[i]*h2_optical_depth_approx/(gloverabel08_h2lte[i]/(H2_1*gloverabel08_gaH2[i] + H_1*gloverabel08_gaHI[i] + H_2*gloverabel08_gaHp[i] + He_1*gloverabel08_gaHe[i] + de*gloverabel08_gael[i]) + 1.0) - H_1*ceHI_ceHI[i]*de - H_1*ciHI_ciHI[i]*de - H_2*de*reHII_reHII[i] - He_1*ciHeI_ciHeI[i]*de - He_2*ceHeII_ceHeII[i]*de - He_2*ceHeI_ceHeI[i]*pow(de, 2) - He_2*ciHeII_ciHeII[i]*de - He_2*ciHeIS_ciHeIS[i]*pow(de, 2) - He_2*de*reHeII1_reHeII1[i] - He_2*de*reHeII2_reHeII2[i] - He_3*de*reHeIII_reHeIII[i] - brem_brem[i]*de*(H_2 + He_2 + 4.0*He_3) - compton_comp_[i]*de*pow(z + 1.0, 4)*(T - 2.73*z - 2.73) + 0.5*1.0/(h2formation_ncrn[i]/(H2_1*h2formation_ncrd2[i] + H_1*h2formation_ncrd1[i]) + 1.0)*(-H2_1*H_1*h2formation_h2mcool[i] + pow(H_1, 3)*h2formation_h2mheat[i]))*fmin(1.00000000000000, (1.0 - exp(-fmax(1.00000000000000e-5, 8.87484891418606e+20*pow(mdensity, 2.8))))/fmax(1.00000000000000e-5, 8.87484891418606e+20*pow(mdensity, 2.8)));
        NV_Ith_S( ydot, j ) *= inv_scale[j];
        
        NV_Ith_S( ydot, j ) *= inv_mdensity;
        
        NV_Ith_S( ydot, j ) *= data->cie_optical_depth_approx[i];
...
int klu_solve_chemistry_dt( dengo_field_data *field_data, 
klu_data *data, double dt ){
             ...
             void *user_data, N_Vector tmp)
{
    ...
    // Species: ge
    //
    Ith(Jv, 10 ) = v0*(-H2_1*gloverabel08_gaH2[i]*pow(gloverabel08_h2lte[i], 2)*h2_optical_depth_approx/(pow(gloverabel08_h2lte[i]/(H2_1*gloverabel08_gaH2[i] + H_1*gloverabel08_gaHI[i] + H_2*gloverabel08_gaHp[i] + He_1*gloverabel08_gaHe[i] + de*gloverabel08_gael[i]) + 1.0, 2)*pow(H2_1*gloverabel08_gaH2[i] + H_1*gloverabel08_gaHI[i] + H_2*gloverabel08_gaHp[i] + He_1*gloverabel08_gaHe[i] + de*gloverabel08_gael[i], 2)) - 0.5*H_1*h2formation_h2mcool[i]*1.0/(h2formation_ncrn[i]/(H2_1*h2formation_ncrd2[i] + H_1*h2formation_ncrd1[i]) + 1.0) - 2.01588*cie_cooling_cieco[i]*mdensity - gloverabel08_h2lte[i]*h2_optical_depth_approx/(gloverabel08_h2lte[i]/(H2_1*gloverabel08_gaH2[i] + H_1*gloverabel08_gaHI[i] + H_2*gloverabel08_gaHp[i] + He_1*gloverabel08_gaHe[i] + de*gloverabel08_gael[i]) + 1.0) + 0.5*h2formation_ncrd2[i]*h2formation_ncrn[i]*pow(h2formation_ncrn[i]/(H2_1*h2formation_ncrd2[i] + H_1*h2formation_ncrd1[i]) + 1.0, -2.0)*(-H2_1*H_1*h2formation_h2mcool[i] + pow(H_1, 3)*h2formation_h2mheat[i])/pow(H2_1*h2formation_ncrd2[i] + H_1*h2formation_ncrd1[i], 2))*fmin(1.00000000000000, (1.0 - exp(-fmax(1.00000000000000e-5, 8.87484891418606e+20*pow(mdensity, 2.8))))/fmax(1.00000000000000e-5, 8.87484891418606e+20*pow(mdensity, 2.8)))/mdensity + v2*(-H2_1*gloverabel08_gaHI[i]*pow(gloverabel08_h2lte[i], 2)*h2_optical_depth_approx/(pow(gloverabel08_h2lte[i]/(H2_1*gloverabel08_gaH2[i] + H_1*gloverabel08_gaHI[i] + H_2*gloverabel08_gaHp[i] + He_1*gloverabel08_gaHe[i] + de*gloverabel08_gael[i]) + 1.0, 2)*pow(H2_1*gloverabel08_gaH2[i] + H_1*gloverabel08_gaHI[i] + H_2*gloverabel08_gaHp[i] + He_1*gloverabel08_gaHe[i] + de*gloverabel08_gael[i], 2)) - ceHI_ceHI[i]*de - ciHI_ciHI[i]*de + 0.5*h2formation_ncrd1[i]*h2formation_ncrn[i]*pow(h2formation_ncrn[i]/(H2_1*h2formation_ncrd2[i] + H_1*h2formation_ncrd1[i]) + 1.0, -2.0)*(-H2_1*H_1*h2formation_h2mcool[i] + pow(H_1, 3)*h2formation_h2mheat[i])/pow(H2_1*h2formation_ncrd2[i] + H_1*h2formation_ncrd1[i], 2) + 0.5*(-H2_1*h2formation_h2mcool[i] + 3*pow(H_1, 2)*h2formation_h2mheat[i])*1.0/(h2formation_ncrn[i]/(H2_1*h2formation_ncrd2[i] + H_1*h2formation_ncrd1[i]) + 1.0))*fmin(1.00000000000000, (1.0 - exp(-fmax(1.00000000000000e-5, 8.87484891418606e+20*pow(mdensity, 2.8))))/fmax(1.00000000000000e-5, 8.87484891418606e+20*pow(mdensity, 2.8)))/mdensity + v3*(-H2_1*gloverabel08_gaHp[i]*pow(gloverabel08_h2lte[i], 2)*h2_optical_depth_approx/(pow(gloverabel08_h2lte[i]/(H2_1*gloverabel08_gaH2[i] + H_1*gloverabel08_gaHI[i] + H_2*gloverabel08_gaHp[i] + He_1*gloverabel08_gaHe[i] + de*gloverabel08_gael[i]) + 1.0, 2)*pow(H2_1*gloverabel08_gaH2[i] + H_1*gloverabel08_gaHI[i] + H_2*gloverabel08_gaHp[i] + He_1*gloverabel08_gaHe[i] + de*gloverabel08_gael[i], 2)) - brem_brem[i]*de - de*reHII_reHII[i])*fmin(1.00000000000000, (1.0 - exp(-fmax(1.00000000000000e-5, 8.87484891418606e+20*pow(mdensity, 2.8))))/fmax(1.00000000000000e-5, 8.87484891418606e+20*pow(mdensity, 2.8)))/mdensity + v5*(-H2_1*gloverabel08_gaHe[i]*pow(gloverabel08_h2lte[i], 2)*h2_optical_depth_approx/(pow(gloverabel08_h2lte[i]/(H2_1*gloverabel08_gaH2[i] + H_1*gloverabel08_gaHI[i] + H_2*gloverabel08_gaHp[i] + He_1*gloverabel08_gaHe[i] + de*gloverabel08_gael[i]) + 1.0, 2)*pow(H2_1*gloverabel08_gaH2[i] + H_1*gloverabel08_gaHI[i] + H_2*gloverabel08_gaHp[i] + He_1*gloverabel08_gaHe[i] + de*gloverabel08_gael[i], 2)) - ciHeI_ciHeI[i]*de)*fmin(1.00000000000000, (1.0 - exp(-fmax(1.00000000000000e-5, 8.87484891418606e+20*pow(mdensity, 2.8))))/fmax(1.00000000000000e-5, 8.87484891418606e+20*pow(mdensity, 2.8)))/mdensity + v6*(-brem_brem[i]*de - ceHeII_ceHeII[i]*de - ceHeI_ceHeI[i]*pow(de, 2) - ciHeII_ciHeII[i]*de - ciHeIS_ciHeIS[i]*pow(de, 2) - de*reHeII1_reHeII1[i] - de*reHeII2_reHeII2[i])*fmin(1.00000000000000, (1.0 - exp(-fmax(1.00000000000000e-5, 8.87484891418606e+20*pow(mdensity, 2.8))))/fmax(1.00000000000000e-5, 8.87484891418606e+20*pow(mdensity, 2.8)))/mdensity + v7*(-4.0*brem_brem[i]*de - de*reHeIII_reHeIII[i])*fmin(1.00000000000000, (1.0 - exp(-fmax(1.00000000000000e-5, 8.87484891418606e+20*pow(mdensity, 2.8))))/fmax(1.00000000000000e-5, 8.87484891418606e+20*pow(mdensity, 2.8)))/mdensity + v8*(-H2_1*gloverabel08_gael[i]*pow(gloverabel08_h2lte[i], 2)*h2_optical_depth_approx/(pow(gloverabel08_h2lte[i]/(H2_1*gloverabel08_gaH2[i] + H_1*gloverabel08_gaHI[i] + H_2*gloverabel08_gaHp[i] + He_1*gloverabel08_gaHe[i] + de*gloverabel08_gael[i]) + 1.0, 2)*pow(H2_1*gloverabel08_gaH2[i] + H_1*gloverabel08_gaHI[i] + H_2*gloverabel08_gaHp[i] + He_1*gloverabel08_gaHe[i] + de*gloverabel08_gael[i], 2)) - H_1*ceHI_ceHI[i] - H_1*ciHI_ciHI[i] - H_2*reHII_reHII[i] - He_1*ciHeI_ciHeI[i] - He_2*ceHeII_ceHeII[i] - 2*He_2*ceHeI_ceHeI[i]*de - He_2*ciHeII_ciHeII[i] - 2*He_2*ciHeIS_ciHeIS[i]*de - He_2*reHeII1_reHeII1[i] - He_2*reHeII2_reHeII2[i] - He_3*reHeIII_reHeIII[i] - brem_brem[i]*(H_2 + He_2 + 4.0*He_3) - compton_comp_[i]*pow(z + 1.0, 4)*(T - 2.73*z - 2.73))*fmin(1.00000000000000, (1.0 - exp(-fmax(1.00000000000000e-5, 8.87484891418606e+20*pow(mdensity, 2.8))))/fmax(1.00000000000000e-5, 8.87484891418606e+20*pow(mdensity, 2.8)))/mdensity;
    ...
    */
    ...
    for ( i = 0; i < nstrip; i++ ){
        ...
        matrix_data[j + 56] *= inv_mdensity;
         ...
         matrix_data[j + 56] *= data->cie_optical_depth_approx[i]; 
        ...
        matrix_data[j + 57] *= inv_mdensity;
         ...
         matrix_data[j + 57] *= data->cie_optical_depth_approx[i]; 
        ...
        matrix_data[j + 58] *= inv_mdensity;
         ...
         matrix_data[j + 58] *= data->cie_optical_depth_approx[i]; 
        ...
        matrix_data[ j + 59 ] = (-H2_1*gloverabel08_gaHe[i]*pow(gloverabel08_h2lte[i], 2)*h2_optical_depth_approx/(pow(gloverabel08_h2lte[i]/(H2_1*gloverabel08_gaH2[i] + H_1*gloverabel08_gaHI[i] + H_2*gloverabel08_gaHp[i] + He_1*gloverabel08_gaHe[i] + de*gloverabel08_gael[i]) + 1.0, 2)*pow(H2_1*gloverabel08_gaH2[i] + H_1*gloverabel08_gaHI[i] + H_2*gloverabel08_gaHp[i] + He_1*gloverabel08_gaHe[i] + de*gloverabel08_gael[i], 2)) - ciHeI_ciHeI[i]*de)*fmin(1.00000000000000, (1.0 - exp(-fmax(1.00000000000000e-5, 8.87484891418606e+20*pow(mdensity, 2.8))))/fmax(1.00000000000000e-5, 8.87484891418606e+20*pow(mdensity, 2.8)));
        
        
        matrix_data[j + 59] *= inv_mdensity;
        
         matrix_data[j + 59] *= data->cie_optical_depth_approx[i]; 
        ...
        matrix_data[j + 60] *= inv_mdensity;
         ...
         matrix_data[j + 60] *= data->cie_optical_depth_approx[i]; 
        ...
        matrix_data[j + 61] *= inv_mdensity;
         ...
         matrix_data[j + 61] *= data->cie_optical_depth_approx[i]; 
        ...
        matrix_data[j + 62] *= inv_mdensity;
         ...
         matrix_data[j + 62] *= data->cie_optical_depth_approx[i]; 
        ...
        matrix_data[ j + 63 ] = (-2.01588*H2_1*cie_cooling_cieco[i]*mdensity - H2_1*gloverabel08_h2lte[i]*h2_optical_depth_approx/(gloverabel08_h2lte[i]/(H2_1*gloverabel08_gaH2[i] + H_1*gloverabel08_gaHI[i] + H_2*gloverabel08_gaHp[i] + He_1*gloverabel08_gaHe[i] + de*gloverabel08_gael[i]) + 1.0) - H_1*ceHI_ceHI[i]*de - H_1*ciHI_ciHI[i]*de - H_2*de*reHII_reHII[i] - He_1*ciHeI_ciHeI[i]*de - He_2*ceHeII_ceHeII[i]*de - He_2*ceHeI_ceHeI[i]*pow(de, 2) - He_2*ciHeII_ciHeII[i]*de - He_2*ciHeIS_ciHeIS[i]*pow(de, 2) - He_2*de*reHeII1_reHeII1[i] - He_2*de*reHeII2_reHeII2[i] - He_3*de*reHeIII_reHeIII[i] - brem_brem[i]*de*(H_2 + He_2 + 4.0*He_3) - compton_comp_[i]*de*pow(z + 1.0, 4)*(T - 2.73*z - 2.73) + 0.5*1.0/(h2formation_ncrn[i]/(H2_1*h2formation_ncrd2[i] + H_1*h2formation_ncrd1[i]) + 1.0)*(-H2_1*H_1*h2formation_h2mcool[i] + pow(H_1, 3)*h2formation_h2mheat[i]))*fmin(1.00000000000000, (1.0 - exp(-fmax(1.00000000000000e-5, 8.87484891418606e+20*pow(mdensity, 2.8))))/fmax(1.00000000000000e-5, 8.87484891418606e+20*pow(mdensity, 2.8)));
        ...
        matrix_data[j + 63] *= inv_mdensity;
         ...
         matrix_data[j + 63] *= data->cie_optical_depth_approx[i]; 
...
void setting_up_extra_variables( klu_data * data, double * input, int nstrip ){
    ...
    for ( i = 0; i < nstrip; i++){
        ...
        // species: H2_1
        data->mdensity[i] += input[j] * 2.01588; 
