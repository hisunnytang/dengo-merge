...
cdef extern from "klu_solver.h":
    ...
    klu_data *klu_setup_data(int *NumberOfFields,
            char ***FieldNames)
    ...
    int calculate_jacobian_klu(long int N, realtype t,
                   N_Vector y, N_Vector fy, DlsMat J, void *user_data,
                   N_Vector tmp1, N_Vector tmp2, N_Vector tmp3);

    int calculate_rhs_klu(realtype t, N_Vector y, 
                    N_Vector ydot, void *user_data);
    
    void klu_calculate_temperature(klu_data *data, double *input, int nstrip, int nchem);
...
def run_klu(ics, double tf, int niter = 10000,
    ...
    cdef klu_data *data = klu_setup_data(NULL, NULL)
    cdef rhs_f f = calculate_rhs_klu
    cdef jac_f jf = calculate_jacobian_klu

    cdef double ttot = 0.0
    cdef int status
    # Allocate some temporary data
    # Now we manually evolve
    #ttot = dengo_evolve_klu(tf, dt, input, rtol, atol, dims, data)
    data.current_z = z
    ...
    for iter in range(niter):
        dt_local = dengo_evolve_klu( dt0[0], dt[0], z, input, rtol, atol, dims, data, temp  )
        if dt_local > 0:
            status = 0
