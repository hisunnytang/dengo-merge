...
cdef extern from "dengo_original_solver.h":
    cdef int _MAX_NCELLS  "MAX_NCELLS"
    cdef int _NSPECIES "NSPECIES"
    ctypedef struct dengo_original_data:
        double dbin
        double idbin
        double bounds[2]
        int nbins
    ...
    dengo_original_data *dengo_original_setup_data(int *NumberOfFields,
            char ***FieldNames)
...
def main_run_dengo_original():
    t1 = time.time()
    dengo_original_main(0, NULL)
    t2 = time.time()
    print "Total elapsed time: %0.3e" % (t2-t1)

def run_dengo_original(ics, double tf, int niter = 10000,
    ...
    cdef dengo_original_data *data = dengo_original_setup_data(NULL, NULL)
    cdef rhs_f f = calculate_rhs_dengo_original
    cdef jac_f jf = calculate_jacobian_dengo_original
    ...
    #ttot = dengo_evolve_dengo_original(tf, dt, input, rtol, atol, dims, data)
    data.current_z = z
